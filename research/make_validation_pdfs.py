...
with PdfPages(p6) as pdf:
    ...
    table_page(pdf, "質の比較（ボラ10%/年に正規化・34ヶ月）",
        ...
        note="Sharpe/勝率は改善(滑らかになる)が、Calmarは単調悪化＝E5を混ぜるほど最大DDが"
             "リターン以上に増える。",
        col_w=[0.22, 0.13, 0.13, 0.13, 0.13, 0.12, 0.14], hl=[1])

    table_page(pdf, "★最大DD予算を同一にした時の年率（プロップの肝）",
        ["配分 v7:E5", "同一DDでの年率（対 既存EA）"],
        [["100:0 （既存EA）", "基準 ±0%"],
         ["75:25", "−38%"],
         ["65:35", "−51%"],
         ["50:50", "−64%"]],
        note="プロップは最大DDで失格が決まる＝『同じ最大DDで何%稼げるか』が全て。"
             "同じDD枠の中ではE5を足すほど期待リターンは減る＝プロップ目的では合成は不利。"
             "E5の価値は保険(分散)で、デモで低相関・正期待値の持続が確認できて初めて成立する。",
        col_w=[0.30, 0.70], hl=[1])
