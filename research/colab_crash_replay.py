...
EVENTS = ["2008-09-15","2008-10-24","2010-05-06","2011-08-08","2015-01-15","2015-08-24",
          "2016-07-06","2018-02-05","2018-11-12","2020-03-12","2020-03-19",
...
SEAS_W = {"2024-08-05": dict(v4=.331, EMon=.206, E5=.328, SJul=.135),
          ...
          "2008-09-15": dict(v4=.483, EMon=.517),
          "2008-10-24": dict(EMon=.379, EMonX=.437, SJul=.184),
          "2010-05-06": dict(v4=.092, EMon=.084, EMonX=.114, SJul=.162),  # 5月のv7系.549は月曜のみ=木曜は非対象
          "2011-08-08": dict(v4=.331, EMon=.206, E5=.328, SJul=.135),
          "2015-01-15": dict(v4=.178, EMon=.478, SJul=.345),
          "2015-08-24": dict(v4=.331, EMon=.206, E5=.328, SJul=.135)}
SEAS_MULT = 1.5
...
def h1(name):
    ...
    if os.path.exists(p):
        ...
        pre=_synth_h1(name, until=df.index.min().tz_localize(None))   # H1開始前は常に擬似H1で補完
        if len(pre):
...
def daily(name):
    ...
    if df is None or len(df[df.index<pd.Timestamp("2016-01-01")])<50 \
       or len(df[df.index<pd.Timestamp("2024-08-01")])<400:
        try:
            y=_yahoo(YH.get(name,name+"=X"),"1d","2000-01-01","2025-05-01")
...
def replay(config, event, slip, seasonal=False):
    ...
    for l in legs:
        if l.alive:
            w=h1(l.name)[0]; w=w[w.index<d1]
            breakdown[l.tag]=round((l.pnl(float(w["close"].iloc[-1])) if len(w) else 0.0)*100,3)
        else:
            breakdown[l.tag]=round(l.realized*100,3)
...
def main():
    ...
    for ev in EVENTS:
        out[ev]={}
        slips = SLIPS + ([0.01] if ev < "2016" else [])   # 旧期間はSNB級1.0%も検査
        for slip in slips:
