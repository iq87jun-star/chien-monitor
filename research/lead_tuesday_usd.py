"""
lead_tuesday_usd.py — 「火曜NY午後 USD安ドリフト」リードの性格づけ(発見の深掘り)。

edge_search_session.py が、v6エッジの抽象化(=曜日×セッション時刻のフロー・ドリフト)を
非JPYに拡張して見つけたリード: 火曜 18-19 UTC に AUD/NZD/GBP/EUR USD が同時LONG
(=USD安)。本スクリプトはこれを「USD安バスケット」に束ね、v6と同じ厳格ハーネスで
性格づける。目的は『円月曜と無相関の第2エッジ候補か』の判定。

⚠ committed H1 = 2.8年のみ。よって「リード(有望)」止まりで、断定はしない。要10年H1追認。
