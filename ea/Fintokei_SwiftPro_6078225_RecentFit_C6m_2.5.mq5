...
//|   RecentFit C6m v1.03 — 2026-08-14 コードレビュー反映版            |
...
//|  倍率2.5・ロック5.9/6.05・MC: 直近合格54.5%(中央31日)/悲観26.7%     |
...
//|   ⑨任意: InpDayProfitCloseAllPct — 1日利益上限3%が「超過で失格」の |
//|     ハード規則である場合に備えた日次+n%全決済(既定0=無効。         |
//|     規則がカウント制(超過分が目標不算入なだけ)なら無効のままで可。 |
//|     docs/183で規則の性質を確認の上設定すること)                    |
//|   ⑩MagicBase 943700→943800(旧RecentFit版とのMagic/GV衝突回避)     |
//+------------------------------------------------------------------+
#property copyright "chien-monitor research"
#property version   "1.03"
#property strict
#property description "[RecentFit C6m Fintokei 6078225] Mon GBPJPY+AUDJPY / v4 NZDUSD+AUDUSD. mult 2.5. Floor -2.4 (tick), daily -1.2 stop / -1.5 close (UTC0, max(bal,eq) anchor), day profit cap +2.5, lock 5.9/6.05. Expiry 2026-10-13 UTC."
...
input double InpDayProfitCloseAllPct = 0.0; // 日次+この%で全決済(0=無効。上限3%がハード規則なら2.9推奨)
...
input group "=== 利益ロック(Fintokei速攻プロ 目標6%: ARM 5.9 / LOCK 6.05) ==="
...
void OnTimer()
{
   ...
   // v1.03(⑨): 利益上限がハード規則の場合の日次全決済(既定0=無効)
...
//|    docs/183で要確認。ハードなら InpDayProfitCloseAllPct=2.9 を設定。|
//|  ・利益ロック(+6.05%)はOnTimer(30秒)評価=急伸時は多少オーバー      |
//|    シュートするが超過方向なので安全側。                            |
//+------------------------------------------------------------------+
