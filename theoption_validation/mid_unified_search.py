...
M = 11*24*5*6*2 = 15,840  α = 0.05/M = 3.16e-06
...
ac.to_csv(SCRATCH/'mid_unified_all.csv', index=False, encoding='utf-8-sig')
f = pd.DataFrame(finals)
if len(f): f = f.sort_values('edge_adj', ascending=False)
f.to_csv(SCRATCH/'mid_unified_final.csv', index=False, encoding='utf-8-sig')
