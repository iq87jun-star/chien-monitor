...
#property strict
#property description "[FN100k P2 5x target+5%] Portfolio-D ONE-CLICK (PROP median-3 standard): v4+v7+E-Mon[RG3]+E5=30:25:25:20. Drop on chart & OK. v1.10 profit lock+risk guard+swap log / v1.20 push notify (Even G2)+persistent baseline+E5 re-enter (docs/112) / v1.30 month-trail lock T2 (docs/116, default OFF)."
...
input double InpNotifyRefPct     = 3.5;   // 手決済の参考値: equity+この%で「検討ライン」通知
...
input double InpProfitStopPct    = 5.0;   // +この%で新規停止(FN P1=8.0)
...
input double InpLockArmPct       = 4.0;   // equity+この%で新規停止(既存+8%停止より手前・水準判定)
input double InpLockClosePct     = 4.5;   // equity+この%で全決済し恒久ロック(PASS_LOCK)
