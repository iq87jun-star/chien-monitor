...
//|       失格』競争に単調有利。                                       |
//|                                                                   |
//|   ★★ 10年実データ検証(ユーザー提供Colab・実bid/ask・docs/18):     |
//|     v7のエッジと「多ショット分散」は10年でも CONFIRM された:        |
//|       ・曜日プラセボ: EURJPY 月曜 p=0.0000(他曜日全て非有意),       |
//|         GBPJPY 月 p=0.0037 → エッジは本物。                       |
//|       ・ショット数 3→12 で SR 2.69→7.26 / maxDD 18.0%→14.8%        |
//|         (=分散は実在。多ショットは正しい)。                       |
//|     ★しかし【最大DDの絶対値】が 2.8年(私の旧主張 -4.9%)では大幅に   |
//|       過小評価だった。10年実測:                                    |
//|       ・週次予算1.5%(旧既定): 全期間maxDD ~14.8% / 年次最悪 ~10.5% |
//|         → ガード有り通しは -8%停止=FundedNext FAIL。MC合格79.2%。  |
//|       ・原因: 2023-26は円が一方向で穏やか。10年の真のDDはその約3倍。|
//|     ⇒ 修正: エッジは活かし【週次予算を下げて器に収める】。maxDDは    |
//|        予算にほぼ線形。新既定 InpWeeklyRiskPct=0.60(全期間maxDD     |
//|        ~5.9% / 年次最悪 ~4.2% と外挿)。攻めるなら0.75(~7.4%)まで。  |
//|        FundedNextは時間無制限ゆえ、低予算で到達が遅れてもDD失格を    |
//|        避ける方が合格率が高い。                                    |
//|                                                                   |
...
//|   ★ まず保守(0.50-0.60)でデモ前進検証 → 10年DDが実口座で再現する    |
//|     か確認してから funded へ。詳細は docs/16 と docs/18。          |
//+------------------------------------------------------------------+
...
input double InpWeeklyRiskPct    = 0.60;    // ★ポートフォリオ週次リスク予算%(全インスタンス共通)
                                            //   ★10年実測で改定: 旧1.5は10年maxDD~14.8%でFundedNext失格。
                                            //   新既定0.60(10年maxDD~5.9%/年次最悪~4.2%と外挿)。
                                            //   攻めるなら0.75(~7.4%)。1.0超は10年DDが10%に接近→非推奨。
