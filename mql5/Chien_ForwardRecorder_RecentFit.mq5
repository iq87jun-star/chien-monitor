//+------------------------------------------------------------------+
//|          Chien_ForwardRecorder_RecentFit.mq5                     |
//|   フォワード記録専用版(docs/218)— 実口座用ではない                |
//|                                                                   |
//|   ★目的: デモ1口座で RecentFit 系の全レグ(Mon/v4/Hold)を同時に    |
//|     走らせ、レグ別の実約定系列を記録する。ポートフォリオ成績は     |
//|     この記録から後処理で任意の重みで合成する(docs/218 §1)。       |
//|                                                                   |
//|   ロジックは 521100397 版 v1.03 と完全に同一。差分は3点のみ:      |
//|     1) MAXLEG 8 → 16(Mon13+v4 9+Hold6 を1インスタンスに収める)   |
//|     2) エントリー/スプレッド見送りを CSV に記録(docs/218 §8.3)   |
//|        → キャップを緩く記録し、任意のキャップを後処理で当てる     |
//|     3) 記録用の入力(InpRecordCsv / InpRecordTag)を追加            |
//|     4) PipOf() を非FX対応に(docs/182 §5 の既存修正を移植)        |
//|     5) v1.20: 曜日×方向セル(InpDowLegs)を追加。docs/229/230 の  |
//|        候補(MonS EURGBP / FriS NZDUSD / WedS・TueS CADCHF /       |
...
//|     他レグの損益がロット計算に混入しないため=口座分割と等価。     |
//|                                                                   |
//|   CSV: MQL5/Files/<InpRecordTag>_entries.csv                      |
//|     utc,event,family,symbol,hour,side,lots,notional,spread_pips,  |
//|     cap_pips,ask,bid,sl,tp,note                                   |
...
input string InpMonLegs  = "USDJPY:0.321,EURJPY:0.277"; // Mon: 月曜o2o LONG(2026-09規則適合版)
...
input datetime InpExpiry = D'2026.09.30 23:59';  // D案=毎月再スクリーニング
...
input string InpMonSpreadCaps = "USDJPY:2.0,EURJPY:2.9"; // ペア別上限(EURJPYは他JPYクロス準拠・暫定/要実測)
...
input long   InpMagicBase = 943400;  // Mon=+1/v4=+2/Hold=+3(旧D版と同一=残存建玉引継ぎ)
input int    InpSlippagePoints = 30;
input bool   InpVerboseLog = true;

input group "=== フォワード記録(docs/218 §8.3) ==="
input bool   InpRecordCsv    = true;    // エントリー/見送りを CSV に書く
input string InpRecordTag    = "rec1";  // 出力= MQL5/Files/<tag>_entries.csv(チャート毎に別名にする)

//==================================================================
CTrade        trade;
CPositionInfo posinfo;

#define MAXLEG 16   // 記録版: Mon13 + v4 9 + Hold6 を1インスタンスで回す(docs/218 §3)
...
string ResolveSymbol(string want)
{
   ...
   if(StringFind(U,"GER")>=0 || StringFind(U,"DE40")>=0 || StringFind(U,"DAX")>=0){    // D案9月版: GER40別名
      bases[nb++]="GER40"; bases[nb++]="DE40"; bases[nb++]="DAX40"; bases[nb++]="GER30"; }
...
// v1.20: "Tag:SYM:W" 形式。Tag = Mon/Tue/Wed/Thu/Fri (+S=SHORT)
int ParseDowLegs(string csv)
{
   ...
   for(int i=0;i<n && k<MAXLEG;i++){
      string kv[]; if(StringSplit(parts[i],':',kv)!=3) continue;
      string tag=kv[0]; StringTrimLeft(tag); StringTrimRight(tag);
      string sym=kv[1]; StringTrimLeft(sym); StringTrimRight(sym);
      double w=StringToDouble(kv[2]); if(StringLen(tag)<3 || StringLen(sym)==0 || w<=0) continue;
      bool sh=(StringSubstr(tag,StringLen(tag)-1,1)=="S" && StringLen(tag)==4);
      string dn=StringSubstr(tag,0,3); int day=-1;
      for(int d=0;d<5;d++) if(dn==names[d]){ day=d+1; break; }      // MQL: Sun=0..Sat=6
      if(day<0){ PrintFormat("⚠ Dow: 曜日タグ不正 '%s'",tag); continue; }
...
double PipOf(string s){
   // 記録版: 非FX(指数/暗号/商品)対応 — docs/182 §5 の修正を移植。pip=point×10。
   // 5桁/3桁FXでは従来値(0.0001/0.01)と一致する。一律0.0001だと GER40/ETHUSD の
   // Monスプレッド判定が恒久超過になり、そのレグが永久に建たない。
   double pt=SymbolInfoDouble(s,SYMBOL_POINT);
   if(pt>0) return pt*10.0;
   return (StringFind(s,"JPY")>=0)? 0.01 : 0.0001; }

//===== フォワード記録CSV(docs/218 §8.3) =====================================
void Rec(string ev,string fam,string sym,int hour,string side,double lots,double notional,
         double spr,double cap,double ask,double bid,double sl,double tp,string note)
{
   if(!InpRecordCsv) return;
   string fn=InpRecordTag+"_entries.csv";
   int h=FileOpen(fn,FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ|FILE_SHARE_WRITE,',');
   if(h==INVALID_HANDLE){ PrintFormat("⚠ 記録CSVを開けません: %s err=%d",fn,GetLastError()); return; }
   if(FileSize(h)==0)
      FileWrite(h,"utc","event","family","symbol","hour","side","lots","notional",
                  "spread_pips","cap_pips","ask","bid","sl","tp","note");
   FileSeek(h,0,SEEK_END);
   FileWrite(h,TimeToString(TimeGMT(),TIME_DATE|TIME_SECONDS),ev,fam,sym,IntegerToString(hour),side,
               DoubleToString(lots,2),DoubleToString(notional,0),
               DoubleToString(spr,2),DoubleToString(cap,2),
               DoubleToString(ask,5),DoubleToString(bid,5),DoubleToString(sl,5),DoubleToString(tp,5),note);
   FileClose(h);
}
double SprPips(string sym){ double a=SymbolInfoDouble(sym,SYMBOL_ASK),b=SymbolInfoDouble(sym,SYMBOL_BID);
   if(a<=0||b<=0) return -1.0; return (a-b)/PipOf(sym); }
//============================================================================
...
int OnInit()
{
   ...
   ArrayInitialize(g_lastShotMon,0); ArrayInitialize(g_monSkipBar,0); ArrayInitialize(g_lastV4Bar,0); ArrayInitialize(g_lastHoldTry,0);
...
void EntriesMon(datetime utc)
{
   ...
   for(int s=0;s<g_nMon;s++){
      ...
      if(spr>cap){
         if(g_monSkipBar[key]!=hourBar){      // 記録版: 同一ショット枠で1回だけ記録
            g_monSkipBar[key]=hourBar;
            Rec("SKIP_SPREAD","Mon",sym,g_monHours[slot],"LONG",0,0,spr,cap,ask,bid,0,0,"");
            if(InpVerboseLog) PrintFormat("[Mon SKIP spread] %s h%dUTC spread=%.2fpip > cap=%.2f",
                                          sym,g_monHours[slot],spr,cap);
         }
         continue;
      }
      ...
      if(trade.Buy(lots,sym,0.0,sl,0.0,StringFormat("RFMon_%s_h%d",sym,g_monHours[slot])))
         { g_lastShotMon[key]=hourBar;
           Rec("ENTRY","Mon",sym,g_monHours[slot],"LONG",lots,notional,spr,cap,ask,bid,sl,0,"");
           if(InpVerboseLog) PrintFormat("[Mon ENTRY] LONG %s h%dUTC lots=%.2f notional=%.0f spread=%.2fpip SL=%.3f",sym,g_monHours[slot],lots,notional,spr,sl);
...
//===== Dow (v1.20: 曜日×方向マルチショット。Mon と同じ時刻・保有・災害SL) =====
void ManageDowExit()
{
   for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0) continue;
      if(!posinfo.SelectByTicket(tk)) continue;
...
void EntriesDow(datetime utc)
{
   ...
   for(int s=0;s<g_nDow;s++){
      if(u.day_of_week!=g_dowDay[s]) continue;
      if(g_atrH1Dow[s]==INVALID_HANDLE) continue;
      int key=s*nh+slot;
      if(g_lastShotDow[key]==hourBar) continue;
      string sym=g_dowSym[s]; double pip=PipOf(sym); bool sh=g_dowShort[s]; string side=(sh?"SHORT":"LONG");
      double atr=AtrAt(g_atrH1Dow[s]); if(atr<=0) continue;
      double sd=InpCatastropheATR*atr; double sp=sd/pip;
      if(sp<InpMinStopPips){ sp=InpMinStopPips; sd=sp*pip; }
      double ask=SymbolInfoDouble(sym,SYMBOL_ASK), bid=SymbolInfoDouble(sym,SYMBOL_BID);
      if(ask<=0||bid<=0) continue;
      double spr=(ask-bid)/pip, cap=SpreadCapFor(sym);
      if(spr>cap){
         if(g_dowSkipBar[key]!=hourBar){
            g_dowSkipBar[key]=hourBar;
            Rec("SKIP_SPREAD",g_dowTag[s],sym,g_monHours[slot],side,0,0,spr,cap,ask,bid,0,0,"");
            if(InpVerboseLog) PrintFormat("[Dow SKIP spread] %s %s h%dUTC spread=%.2fpip > cap=%.2f",g_dowTag[s],sym,g_monHours[slot],spr,cap);
         }
         continue;
      }
      double notional=g_initBal*g_dowW[s]*InpMult/nh;
      double lots=LotsForNotional(sym,notional); if(lots<InpMinLot){ g_lastShotDow[key]=hourBar; continue; }
      int dg=(int)SymbolInfoInteger(sym,SYMBOL_DIGITS);
      string cmt=StringFormat("RF%s_%s_h%d",g_dowTag[s],sym,g_monHours[slot]);
      bool ok; double sl;
      ...
      if(ok){ g_lastShotDow[key]=hourBar;
              Rec("ENTRY",g_dowTag[s],sym,g_monHours[slot],side,lots,notional,spr,cap,ask,bid,sl,0,"");
...
void EntriesV4()
{
   ...
   for(int i=0;i<g_nV4;i++){
      ...
      if(sig>0){ double e=SymbolInfoDouble(sym,SYMBOL_ASK);
         ...
         if(ok){
            Rec("ENTRY","v4",sym,-1,"LONG",lots,notional,SprPips(sym),-1,e,SymbolInfoDouble(sym,SYMBOL_BID),sl,tp,"");
            if(InpVerboseLog) PrintFormat("[v4 ENTRY] LONG %s lots=%.2f notional=%.0f spread=%.2fpip",sym,lots,notional,SprPips(sym));
         ...
         if(ok){
            Rec("ENTRY","v4",sym,-1,"SHORT",lots,notional,SprPips(sym),-1,SymbolInfoDouble(sym,SYMBOL_ASK),e,sl,tp,"");
            if(InpVerboseLog) PrintFormat("[v4 ENTRY] SHORT %s lots=%.2f notional=%.0f spread=%.2fpip",sym,lots,notional,SprPips(sym));
...
void EntriesHold()
{
   ...
   for(int i=0;i<g_nHold;i++){
      ...
      if((ask-bid)/pt>InpHoldMaxSpreadPts){
         g_lastHoldTry[i]=now;
         Rec("SKIP_SPREAD","Hold",sym,-1,"LONG",0,0,(ask-bid)/pt,InpHoldMaxSpreadPts,ask,bid,0,0,"unit=points");
         continue; }
