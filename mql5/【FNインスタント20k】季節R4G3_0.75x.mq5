//+------------------------------------------------------------------+
//|                     Chien_Seasonal_R4G3_Prop.mq5 (INSTANT 20k)   |
//|   ★FN Stellar Instant $20K 専用ワンクリック(docs/154/155):        |
//|     倍率0.75x / 初期残高20,000 / トレーリング−6%(建値ロック)対応。  |
//|   ★使い方: チャートにドロップ → OK。出金/ステップアップ後は       |
//|     InpBaselineReset=true で基準残高とHWMを取り直す。             |
//|   ★校正(docs/154/155・日次MC2万パス):                             |
//|     0.75x = 昇格(+10%)中央8ヶ月 / 昇格成功89.4% / 失格10.6%。      |
//|   ★インスタント適合(P1標準v1.41からの変更点):                     |
//|     1) トレーリングDDガード: FNフロア=min(HWM−6%×初期, 建値)。     |
//|        フロア+2%で新規停止(回復で再開)・+1%で全決済+恒久停止。     |
//|        HWMは端末保存=再起動/再アタッチで引き継ぐ。                 |
//|     2) 全建玉に災害SL付与(月曜系2.5×ATR(D1)/E5・S-Jul 12%)        |
//|        = Max Risk 3%規則(SL基準・トレード個別)を可測化。           |
//|     3) 1トレードのSL損失を2.5%で機械キャップ(3%規則の内側)。       |
//|     4) 利益ロック無効(通過目標なし)。+10%到達通知=出金・昇格処理。  |
//|     5) G3リスク0.8%/イベント(SL2.5ATR→最大損失2.0%<3%)。          |
//|   ⚠ スワップフリー口座前提(docs/154)。デモ/初回ログ確認後に本稼働。 |
//+------------------------------------------------------------------+
#property copyright "chien-monitor research"
#property version   "1.42"   // = 季節P1標準v1.41 + インスタント適合(トレーリング/災害SL/キャップ)
#property strict
...
input double InpMultOverride    = 0.75;   // インスタント昇格レース校正(docs/155)。生存最優先なら0.5

input group "=== ガード(プロジェクト標準) ==="
input double InpDailyStopPct      = 4.0;  // 日次この%負けたら当日停止+全決済
input double InpAccountFloorDDPct = 5.0;  // 静的バックストップ(主ガードはトレーリングDD)
...
input bool   InpProfitLockEnable  = false;// インスタントは通過目標なし=無効(+10%は通知→手動出金)
...
input group "=== トレーリングDDガード(FN Instant −6%・建値ロック, docs/154) ==="
input double InpTrailDDPct      = 6.0;   // FNの最大損失: フロア=min(HWM−この%×初期, 建値)
...
input double InpNoSLRiskAssumePct = 10.0; // SLなし建玉が残った場合の想定損失=建玉額×この%(保守側)
input double InpMaxTradeRiskPct   = 2.5;  // 1トレードのSL損失上限%(FN 3%規則の内側・超過は自動減量)

input group "=== 災害SL(全建玉に付与=Max Risk 3%の可測化) ==="
input double InpCatSLMondayATR  = 2.5;   // 月曜系: エントリ−この×ATR(D1)
input double InpCatSLPct        = 12.0;  // E5/S-Jul: 建値からこの%
...
input double InpG3RiskPct       = 0.8;    // 1イベントのリスク%(SL2.5ATR→最大損失2.0%<3%規則)
...
input double InpNotifyRefPct      = 10.0;   // ステップアップ到達通知: equity+この%(出金→BaselineReset)
...
input long   InpMagicBase       = 940900; // インスタント専用基底(他口座EAと衝突しない)
...
int OnInit()
{
   ...
   // HWM(トレーリングDD基準)の永続化: 再起動/再アタッチで等しく引き継ぐ
   g_gvHwm=StringFormat("ChienSeasonal_hwm_%I64d_%I64d",
                        (long)AccountInfoInteger(ACCOUNT_LOGIN),(long)InpMagicBase);
   {
      ...
      PrintFormat("[HWM] %.2f (FNフロア=min(HWM-%.1f%%x初期, 建値)=%.2f)",
                  g_hwm,InpTrailDDPct,MathMin(g_hwm-g_initBal*InpTrailDDPct/100.0,g_initBal));
   }
...
bool OpenNotional(string s, int sleeve, int dir, double notional, double slPrice=0.0, double tpPrice=0.0, string tag="")
{
   ...
   if(g_trailArmed) return false;                // トレーリングDD: 新規停止(FNフロア+緩衝以下)
   ...
   if(slPrice>0.0 && lots>0 && g_initBal>0){     // 1トレードSL損失キャップ(Max Risk 3%の内側)
      double px0=(dir>0? SymbolInfoDouble(s,SYMBOL_ASK):SymbolInfoDouble(s,SYMBOL_BID));
      double mpu0=MoneyPerUnit(s), dist0=MathAbs(px0-slPrice);
      if(mpu0>0 && dist0>0){
         double maxLoss=g_initBal*InpMaxTradeRiskPct/100.0, atSL=lots*dist0*mpu0;
         if(atSL>maxLoss){
            double vstep=SymbolInfoDouble(s,SYMBOL_VOLUME_STEP);
            double vmin =SymbolInfoDouble(s,SYMBOL_VOLUME_MIN);
            double capL =(vstep>0? MathFloor(maxLoss/(dist0*mpu0)/vstep)*vstep : 0.0);
            PrintFormat("[TRADE RISK CAP] %s lots %.2f->%.2f (SL損失%.0f->上限%.0f=%.1f%%)",
                        s,lots,capL,atSL,maxLoss,InpMaxTradeRiskPct);
            lots=(capL>=vmin? capL : 0.0);
         }
      }
   }
...
//------------------------------------------------------------------ 災害SL用ATR(D1)
double AtrD1(string s, int n=14)
{
   MqlRates r[];
   if(CopyRates(s,PERIOD_D1,1,n+1,r)<n+1) return 0.0;   // [0]=最古(確定足のみ)
   double atr=0.0;
   for(int i=1;i<=n;i++){
      double tr=MathMax(r[i].high-r[i].low,
                 MathMax(MathAbs(r[i].high-r[i-1].close),MathAbs(r[i].low-r[i-1].close)));
      atr=(i==1? tr : (tr+(n-1.0)*atr)/n);
   }
   return atr;
}
...
void SleeveMonday(int sleeve, string &syms[], int nsym, double W, int slot)
{
   ...
   for(int i=0;i<nsym;i++){
      ...
      double atr=AtrD1(syms[i]);
      double px=SymbolInfoDouble(syms[i],SYMBOL_ASK);
      int dg=(int)SymbolInfoInteger(syms[i],SYMBOL_DIGITS);
      double slp=(atr>0&&px>0? NormalizeDouble(px-InpCatSLMondayATR*atr,dg) : 0.0);
      if(OpenNotional(syms[i],sleeve,+1,eq*W*g_mult/nsym,slp,0,
...
void SleeveE5(double W)
{
   ...
   for(int i=0;i<g_ne5;i++){
      ...
      double px=(sgn[i]>0? SymbolInfoDouble(g_e5[i],SYMBOL_ASK):SymbolInfoDouble(g_e5[i],SYMBOL_BID));
      int dg=(int)SymbolInfoInteger(g_e5[i],SYMBOL_DIGITS);
      double slp=(px>0? NormalizeDouble(px*(1.0-sgn[i]*InpCatSLPct/100.0),dg) : 0.0);
      if(OpenNotional(g_e5[i],SL_E5,(int)sgn[i],eq*W*g_mult*invv[i]/sumInv,slp,0,(firstBuild?"E5_TSMOM":"E5_REENTER"))) done++;
   }
...
void SleeveSJul(double W)
{
   ...
   for(int i=0;i<g_nsjul;i++){
      ...
      double px=SymbolInfoDouble(g_sjul[i],SYMBOL_ASK);
      int dg=(int)SymbolInfoInteger(g_sjul[i],SYMBOL_DIGITS);
      double slp=(px>0? NormalizeDouble(px*(1.0-InpCatSLPct/100.0),dg) : 0.0);
      if(OpenNotional(g_sjul[i],SL_SJUL,+1,eq*W*g_mult/g_nsjul,slp,0,(firstBuild?"SJUL":"SJUL_REENTER"))) done++;
   }
...
void OnTimer()
{
   ...
   if(InpSwapLogEnable){
      ...
      if(swDk!=s_swapDayKey){
         ...
         if(g_gvHwm!="")  GlobalVariableSet(g_gvHwm,g_hwm);
   ...
   // トレーリングDDガード(FN Instant: フロア=min(HWM−6%×初期, 建値), docs/154)
   if(eq>g_hwm){ g_hwm=eq; if(g_gvHwm!="") GlobalVariableSet(g_gvHwm,g_hwm); }
   double fnFloor=MathMin(g_hwm-g_initBal*InpTrailDDPct/100.0, g_initBal);
   double haltLvl=fnFloor+g_initBal*InpTrailHaltBufPct/100.0;
   double armLvl =fnFloor+g_initBal*InpTrailArmBufPct/100.0;
   if(eq<=haltLvl && !g_halted){
      g_halted=true; CloseAllMine("TRAIL_FLOOR");
      PrintFormat("[HALT] equity %.2f <= trail停止線%.2f (FNフロア%.2f+緩衝%.1f%%)",eq,haltLvl,fnFloor,InpTrailHaltBufPct);
      Notify(StringFormat("TRAIL_FLOOR 全決済・恒久停止 (eq=%.0f FNフロア=%.0f)",eq,fnFloor)); FlushNotify();
   }
   // 静的フロア(最終バックストップ)
   ...
   bool tArm=(eq<=armLvl);
   if(tArm!=g_trailArmed){
      g_trailArmed=tArm;
      PrintFormat("[TRAIL %s] eq=%.2f FNフロア=%.2f (+%.1f%%線)",(tArm?"ARMED=新規停止":"DISARM=新規再開"),eq,fnFloor,InpTrailArmBufPct);
      Notify(StringFormat("TRAIL %s eq=%.0f (FNフロア=%.0f)",(tArm?"新規停止":"新規再開"),eq,fnFloor)); FlushNotify();
   }
   Comment(StringFormat("Chien_Seasonal_INSTANT20k | eq %+.2f%% | HWM %+.2f%% | FNフロアまで%.2f%% | open-risk %.2f%% (1トレードcap %.1f%%) | %s",
          (g_initBal>0?(eq-g_initBal)/g_initBal*100.0:0.0),
          (g_initBal>0?(g_hwm-g_initBal)/g_initBal*100.0:0.0),
          (g_initBal>0?(eq-fnFloor)/g_initBal*100.0:0.0),
          OpenRiskPct(),InpMaxTradeRiskPct,
          (g_trailArmed?"TRAIL_ARMED(新規停止)":"active")));
   // ステップアップ到達通知(+10%): 出金→スケールアップ申請→InpBaselineReset=true
   if(g_initBal>0 && InpNotifyRefPct>0){
      double gainRef=(eq-g_initBal)/g_initBal*100.0;
      if(!g_ntfRef && gainRef>=InpNotifyRefPct){
         g_ntfRef=true;
         Notify(StringFormat("ステップアップ水準到達 %+.2f%% (出金→昇格→BaselineReset)",gainRef)); FlushNotify();
      }else if(g_ntfRef && gainRef<InpNotifyRefPct-0.5) g_ntfRef=false;
   }
