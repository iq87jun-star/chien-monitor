...
#property version   "1.45"   // = v1.44 + G3 第2銘柄 EURUSD(docs/214/217)
#property strict
#property description "[FN100k #14074882 P1 R4G3 standard v1.45 +G3 EURUSD] R4+G3 ONE-CLICK (median-3, 1.5x, guards+notify+width built-in, v1.40 dual-path lot sizing sanity / v1.41 no-roll-on-restart fix, docs/153). Drop on chart & OK. Demo-first (docs/110-112)."
...
input double InpMultOverride    = 1.0;    // 研究1.0x(docs/172: RG3補正+balanceガード体制。資金化後は再校正)
...
input group "=== RG3 リスクオフ・ゲート(docs/66/68採用・EMon/EMonX月曜に適用) ==="
input bool   InpEMonRegimeGate = true;    // SMA割れ×高ボラの真リスクオフ週は指数月曜を見送り
...
input long   InpMagicBase       = 940900; // 旧Seasonal(930700)と別基底=同居可
...
string   g_g3sym2="";          // v1.45: G3 第2銘柄
int      g_g3atr2=INVALID_HANDLE;
datetime g_g3Entered2=0;
...
int OnInit()
{
   ...
   if(InpG3Enable){
      ...
      if(g_g3sym==""){ Print("⚠ G3: 銘柄解決不可→G3無効"); }
         ...
         if(InpG3Symbol2!=""){                                  // v1.45
            g_g3sym2=ResolveSymbol(InpG3Symbol2);
            if(g_g3sym2=="" || g_g3sym2==g_g3sym){ g_g3sym2=""; Print("⚠ G3-2: 銘柄解決不可/重複→第2銘柄無効"); }
            else{
               g_g3atr2=iATR(g_g3sym2,PERIOD_D1,14);
               PrintFormat("[G3-2] 有効: %s risk=%.1f%%/event(LEAD・要デモ, docs/214/217)",g_g3sym2,InpG3RiskPct2);
            }
         }
      }
   }
...
void OnDeinit(const int reason){ EventKillTimer(); if(g_g3atr!=INVALID_HANDLE) IndicatorRelease(g_g3atr); if(g_g3atr2!=INVALID_HANDLE) IndicatorRelease(g_g3atr2); }
...
void SleeveG3()
{
   ...
   if(w<0){ CloseSleeve(SL_G3,"G3_WINDOW_END"); return; }   // 窓外=手仕舞い(声明5分前の退出を含む・両銘柄)
   SleeveG3Sym(g_g3sym, g_g3atr, InpG3RiskPct, g_g3Entered, w, "G3");
   if(g_g3sym2!="") SleeveG3Sym(g_g3sym2, g_g3atr2, InpG3RiskPct2, g_g3Entered2, w, "G3-2");   // v1.45
}
void SleeveG3Sym(string sym, int atrHandle, double riskPct, datetime &entered, int w, string tag)
{
   if(entered==g_g3WinStart[w]) return;                     // この窓は処理済み(銘柄別)
   if(CountSleeve(SL_G3,sym)>0){ entered=g_g3WinStart[w]; return; }
   if(g_lockDone || g_lockArmed) return;                    // 利益ロック
   if(RiskGuardBlocked(tag)) return;                        // Max Riskガード
   if(SpreadBps(sym)>InpMaxSpreadBps) return;               // 次のタイマーで再試行
   double a[1];
   if(atrHandle==INVALID_HANDLE || CopyBuffer(atrHandle,0,1,1,a)<1) return;
   double atr=a[0]; if(atr<=0) return;
   double mpu=MoneyPerUnit(sym);
   if(mpu<=0) return;
   double riskMoney=g_initBal*riskPct/100.0;
   ...
   if(trade.Buy(lots,sym,0.0,sl,0.0,"G3_FOMC")){
      entered=g_g3WinStart[w];
      if(InpVerboseLog) PrintFormat("[%s ENTRY] LONG %s lots=%.2f SL=%.2f (窓終了=%s)",
         tag,sym,lots,sl,TimeToString(g_g3WinEnd[w],TIME_DATE|TIME_MINUTES));
   }
}
...
void SleeveMonday(int sleeve, string &syms[], int nsym, double W, int slot)
{
   ...
   if(InpEMonRegimeGate && (sleeve==SL_EMON || sleeve==SL_EMONX)){   // v1.44: RG3ゲート(docs/172)
      bool offw=(sleeve==SL_EMON ? RiskOffWeek(g_emon,g_nemon) : RiskOffWeek(g_emonx,g_nemonx));
      if(offw){ g_monWeekKey[slot]=wk;
         PrintFormat("[RG3] %s リスクオフ週(SMA%d割れ×高ボラ)→月曜建て見送り",
                     (sleeve==SL_EMON?"EMon":"EMonX"),InpRG_SMA);
         return; }
   }
   double eq=AccountInfoDouble(ACCOUNT_EQUITY);
   int done=0;
...
void SleeveE5(double W)
{
   ...
   for(int i=0;i<g_ne5;i++){
      ...
      bool balReE5=(g_balFires>0 && !g_balMonthHalt && !BalGuardActive());   // v1.44
      if(!firstBuild && !ManualCloseThisMonth(SL_E5,g_e5[i]) && !balReE5) continue;
...
void SleeveSJul(double W)
{
   ...
   for(int i=0;i<g_nsjul;i++){
      ...
      bool balReSJ=(g_balFires>0 && !g_balMonthHalt && !BalGuardActive());   // v1.44
      if(!firstBuild && !ManualCloseThisMonth(SL_SJUL,g_sjul[i]) && !balReSJ) continue;
...
//===== RG3 リスクオフ・ゲート(docs/66/68・PD版から移植・v1.44) =====
...
bool RiskOffWeek(string &syms[], int nsym)
{
   ...
   for(int s=0;s<nsym;s++){
      double c[]; int got=CopyClose(syms[s],PERIOD_D1,0,need,c);
      if(got<InpRG_SMA+2) continue;
      ArraySetAsSeries(c,true);                        // c[1]=前営業日の確定終値(前足確定)
   ...
   double bv[]; int m2=0; ArrayResize(bv,InpRG_VolLB);
   for(int d=0;d<InpRG_VolLB;d++) if(bvcnt[d]>0) bv[m2++]=bvsum[d]/bvcnt[d];
   if(m2<30) return false;
   double cur=bv[0]; double thr=PercentileSorted(bv,m2,InpRG_VolQ);
   return ((!up_maj) && (cur>thr));                     // 真リスクオフ週=見送り
}
...
void BalGuardCheck()
{
   ...
   if(bdk!=g_dayKey){ g_dayKey=bdk; g_dayStartEq=beq; g_dayHalt=false;
...
void OnTimer()
{
   ...
   if(g_dayHalt || BalGuardActive()) return;
