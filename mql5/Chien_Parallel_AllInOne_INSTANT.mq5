//+------------------------------------------------------------------+
//|                    Chien_Parallel_AllInOne_INSTANT.mq5          |
//|   並行ポートフォリオ・オールインワン(インスタント既定):           |
...
//|                                                                   |
//|   ■ 本ファイル=インスタント既定 ★攻め2.0x(Blueberry等・即時資金化): |
//|     E-Mon週次 1.20% / E5 legRisk 0.60%・−10%トレーリング/無目標。    |
//|     全停止フロア=−9%(枠を使い切る攻め)。気長確実は PORT_MANUAL 1.0x  |
//|     (E-Mon1.00/E5 0.50)。守りは 0.6x(0.60/0.30)。                   |
//|     ⚠ トレーリング枠ゆえDDの瞬間値が効く。攻め2.0xは口座喪失を伴う   |
//|       前傾運用＝利益は毎サイクル必ず出金・口座に大金を置かない。    |
//|     (プロップ審査運用は Chien_Parallel_AllInOne_PROP.mq5 を使う)    |
//|                                                                   |
//|   ■ 内部2戦略(Magic分離で混線なし):                                |
...
#property strict
#property description "Parallel portfolio all-in-one (INSTANT default): E-Mon(equity-index Monday LONG)+E5(multi-asset trend). Per-strategy magics. Both STRONG-LEAD=demo first."
...
input ENUM_PARA_SCENARIO InpScenario = PARA_INSTANT; // ★本ファイル既定=インスタント攻め2.0x
...
input double InpEMonWeeklyPct   = 1.20;   // E-Mon 週次リスク%（INSTANT 2.0x攻め既定と同値）
...
input long   InpMagicBase = 950710;       // E-Mon=+1 / E5=+2 (INSTANT系・PROP 950720と分離)
