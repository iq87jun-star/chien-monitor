...
#property version   "1.43"   // = v1.41 + S-Jul月初値エントリー(統計整合)
#property strict
#property description "[FTMO100k #531343523 R4G3 target+10%] R4+G3 ONE-CLICK (median-3, 1.5x, guards+notify+width built-in, v1.40 dual-path lot sizing sanity / v1.41 no-roll-on-restart fix, docs/153). Drop on chart & OK. Demo-first (docs/110-112)."
...
input double InpLockArmPct        = 9.0;  // equity+この%で新規停止(水準判定・後退で自動再開)
input double InpLockClosePct      = 9.5;  // equity+この%で全決済し恒久ロック(PASS_LOCK)
...
input string InpE5Assets = "XAUUSD,US500,US100,GER40";
input string InpEMon     = "US500,US100,GER40";
...
input string InpSJul     = "US500,US100";
...
input double InpNotifyRefPct      = 8.0;    // 手決済の参考値: equity+この%で「検討ライン」通知
