...
//|   【Fintokeiパール500万円・通常チャレンジ専用】(docs/173)            |
//|   日次=equity基準-5%(書面確認済)・全体静的-10% → 旧型ガードが適合    |
//|   倍率=研究1.25x。日次ガードは旧equity型-4%が担当(balanceガードOFF) |
//|   ⚠Fintokeiの日次リセットはUTC0時=JST9時。初週にログで日界を確認。  |
//|   ⚠フェーズ2(目標6%)ではInpProfitStopPct=6.1/Lock 5.9,6.05に変更。  |
//|   ⚠ 実装は Chien_Seasonal_Top_EA v1.50 の逐語コピー(既定値のみ変更・ |
//|     Magic基底940700=旧EAと同居しても衝突しない)。                   |
//+------------------------------------------------------------------+
#property copyright "chien-monitor research"
#property version   "1.44"   // = v1.41 + S-Jul月初値エントリー(統計整合)
#property strict
...
input double InpMultOverride    = 1.25;   // 研究1.25x(docs/173: Fintokei通常プラン=equity基準日次・到達96.5%/中央8ヶ月)
...
input double InpInitialBalance  = 5000000.0; // パール口座専用(500万円焼き込み・ガード基準を確定)
...
input double InpBalGuardPct      = 0.0;   // Fintokei通常プラン=equity基準のためOFF(旧日次ガード-4%が担当・docs/173)
input int    InpBalGuardMaxMonth = 2;     // 月内発動上限(超過は月末まで新規停止)

input long   InpMagicBase       = 941000; // 旧Seasonal(930700)と別基底=同居可
