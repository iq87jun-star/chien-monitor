...
input string InpMonLegs  = "";                          // Mon: 未使用(EA6 は Sess 単独。1-Step 日次3% で Mon は倍率が出ない・docs/245)
input string InpV4Legs   = "";                          // v4: 未使用
input string InpHoldLegs = "";                          // Hold: 未使用(スワップ有り口座)
input double InpMult     = 0.0;   // Mon 未使用
...
input double InpSessMaxOpenRiskPct  = 2.0;   // Sess 建玉の SL 合計リスク上限(基準残高比%)。日次−2.4% の内側。超える建玉は見送り
input double InpTotalMaxOpenRiskPct = 2.4;   // 全建玉(他EA含む)の SL 合計リスク上限
...
input long   InpMagicBase = 944500;  // Sess=+5(FTMO 既存 EA と衝突しない基底)
...
double   g_eodHwm=0.0;    // v1.13: 日開始時 equity の最高値(EOD トレーリング床用・端末永続)
...
void ResetDay(datetime t){ g_curDay=DayStart(t); g_dayStartEq=AccountInfoDouble(ACCOUNT_EQUITY); g_dayBlocked=false;
   if(g_dayStartEq>g_eodHwm){ g_eodHwm=g_dayStartEq; if(g_gvName!="") GlobalVariableSet(g_gvName+"_hwm",g_eodHwm); }   // v1.13
...
void OnTimer()
{
   ...
   // フロア: 静的(初期残高−10%)と EOD トレーリング(日開始 equity 最高値−10%・+10% 到達で初期残高に固定)の高い方(v1.13)
   ...
   Comment(StringFormat("Chien_EA6_FTMO_1Step | gain %+.2f%% | SessMult %.1f | floor %.0f | %s",gainPct,InpSessMult,floorEq,
...
// v1.13(EA5 由来): 建玉の SL リスク(口座通貨)。SL 未設定は 0 扱い
...
void EntriesSess(datetime utc)
{
   ...
   for(int s=0;s<g_nSes;s++){
      ...
      // v1.13: SL リスク上限(この建玉を足した合計が上限を超えるなら見送り。同時間帯内で再試行はしない)
      { double ts=SymbolInfoDouble(sym,SYMBOL_TRADE_TICK_SIZE), tv=SymbolInfoDouble(sym,SYMBOL_TRADE_TICK_VALUE);
