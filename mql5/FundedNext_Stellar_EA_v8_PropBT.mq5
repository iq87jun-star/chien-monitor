...
CSymbolInfo   sym;
double   g_initBal = 0.0;   // 実効口座サイズ(0入力なら口座残高を自動採用)
...
int OnInit()
{
   ...
   g_trailFloor = g_initBal*(1.0 - g_maxLossPct/100.0);
...
void OnTick()
{
   ...
   double guardFloor  = breachFloor + g_initBal*g_floorBufPct/100.0; // 床の手前で停止
   ...
   if(g_useDailyStop)
   {
      ...
      if(dayPnL<=-g_initBal*g_dailyStopPct/100.0 && !g_dayBlocked)
      {
         g_dayBlocked=true;
...
{
   double riskMoney=g_initBal*(PerShotRiskPct()/100.0);   // ショット当たり=週次予算/ショット数
