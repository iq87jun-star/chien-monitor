...
#property strict
#property description "G3 pre-FOMC drift on US500 (ADOPT, edge12 6/6). Long 24h before FOMC statement, exit 13:55 ET. Demo-first per project discipline."
...
input group "=== リスク ==="
input double InpEventRiskPct      = 1.0;    // 1イベントのリスク予算(ATR(D1)変動≒この%損益)
...
input double InpMaxSpreadBps   = 5.0;       // スプレッドがこれ(bps)超なら見送り(検証コスト基準1bps×安全率)
input long   InpMagic          = 930612;    // edge12/G3
