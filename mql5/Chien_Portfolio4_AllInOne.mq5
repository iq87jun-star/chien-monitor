//+------------------------------------------------------------------+
//|                       Chien_Portfolio4_AllInOne.mq5               |
//|   4戦略オールインワン(加重・プロップ/インスタント両対応):           |
//|   v7(円月曜) + v4(日足k≥4合議) + E5(多資産トレンド) + E-Mon(株価   |
//|   指数月曜) を1チャートに乗せるだけで全銘柄運用。                   |
//|                                                                   |
//|   ■ なぜ4戦略か(docs/50-52): 新候補 E-Mon(米独株価指数の月曜LONG)は |
//|     v7/E5と同級STRONG-LEAD・全既存成分と低相関(v7=0.22/v4=-0.13/    |
//|     E5=-0.15)。現状3種(v7+v4+E5)にE-Monを加えると、★DDを現状に     |
//|     合わせたまま年率が +45〜65% (INSTANT 7.8%→11.3% / PROP 12.3%   |
//|     →17.9%, 加重S1・同DD, docs/53)。相関0.2前後の分散が源泉。       |
//|                                                                   |
//|   ■ シナリオ(InpScenario)＝加重S1(v7:v4:E5:E-Mon=25:35:15:25)・同DD: |
//|     PORT_INSTANT : 2.0x相当・トレーリング−10%/無目標。              |
//|                    v7 1.00% / v4 0.36% / E5 0.60% / E-Mon 1.00%。   |
//|     PORT_PROP    : 3.0x相当・静的−10%/+8%停止/日次−4%。             |
//|                    v7 2.50% / v4 0.90% / E5 1.50% / E-Mon 2.50%。   |
//|     PORT_MANUAL  : 下の手動値・静的−10%(等加重S3にも設定可)。        |
//|     全停止フロア=−9%(枠を使い切る攻め)。守りは InpAccountFloorDDPct=8。|
//|                                                                   |
//|   ■ 内部4戦略(Magic分離 base+1/+2/+3/+4):                          |
//|     v7   : 円3クロス 月曜04/06/08/10UTC LONG・24h(SL=2.5ATR_H1)     |
//|     v4   : 9ペア 日足k≥4合議 両建て・SL1.5ATR_D1/RR1.2/8日          |
//|     E5   : 金+指数 月初TSMOM両建て・逆ボラ(legRisk)/SL2.5ATR_MN1    |
//|     E-Mon: 株価指数3 月曜LONG・24h(SL=3.0ATR_H1)・週次予算/3指数    |
//|     口座ガード: 総合−10%枠の内側(既定−9%)で全停止。                |
//|                                                                   |
//|   ★1チャートに本EAを1つだけ。残高自動。銘柄名は業者仕様に合わせ    |
//|     InpYen/InpV4/InpE5/InpEMon Symbols を編集(US500=SPX500 等)。   |
//|   ⚠ v4=ADOPT / E5・E-Mon=STRONG-LEAD(未確証)。本資金前デモ必須。    |
//|     既定サイズは「同DD・加重」の出発点。E-Monは素DD大ゆえ、デモで    |
//|     合算maxDDが−10%枠内か必ず実測し、超えたらE-Monから削る。        |
//+------------------------------------------------------------------+
#property copyright "chien-monitor research"
#property version   "4.00"
#property strict
#property description "4-strategy all-in-one (weighted, PROP/INSTANT): v7+v4+E5+E-Mon(equity Monday). Same-DD weighting lifts return ~+45-65%. Per-strategy magics. E5/E-Mon=LEAD/demo."

#include <Trade/Trade.mqh>
#include <Trade/PositionInfo.mqh>

enum ENUM_PORT4_SCENARIO
{
   PORT_INSTANT = 0, // インスタント 2.0x相当・加重S1・同DD(トレーリング-10%/無目標)
   PORT_PROP    = 1, // プロップ突破 3.0x相当・加重S1・同DD(静的-10%/+8%/日次-4%)
   PORT_MANUAL  = 2  // 手動(下の Weekly/V4/E5/EMon 値・静的-10%)
};

input group "=== シナリオ（これ1つで倍率/比率/ルール自動・加重S1）==="
input ENUM_PORT4_SCENARIO InpScenario = PORT_PROP; // ★運用先(本ファイルはPROP/INSTANT両対応)
input bool   InpAcknowledgeLEAD = true;   // v4=ADOPT/E5・E-Mon=LEAD。デモ/小サイズで承認=true
...
input string InpEMonSymbols= "US500,NAS100,GER40";                            // E-Mon: 株価指数3
...
input double InpV4RiskPerTradePct= 0.90;  // v4 1トレードあたりリスク%
input double InpE5LegRiskPct     = 1.50;  // E5 legRisk%(各レッグ月次σ)
input double InpEMonWeeklyPct     = 2.50; // E-Mon 週次リスク%(3指数で分割)
...
input int    InpEMonHourUTC   = 0;        // 月曜エントリー時刻UTC(週初・指数寄り目安)
input int    InpEMonHoldHours = 24;       // 24h時間決済
input int    InpEMonAtrH1     = 24;
input double InpEMonCatATR    = 3.0;       // 保険SL=ATR(H1)×倍(時間決済が主)
input double InpEMonMaxSpreadFrac = 0.0015;// 指数の許容スプレッド(価格比, 0.15%)

input group "=== 共通 ==="
input double InpMinLot = 0.01;
input double InpMaxLot = 50.0;
input long   InpMagicBase = 940730;       // v7=+1 / v4=+2 / E5=+3 / E-Mon=+4
...
string   g_yen[]; string g_v4[]; string g_e5[]; string g_emon[]; int g_hours[];
int      g_atrH1[]; int g_atrD1[]; int g_rsiD1[]; int g_atrMN1[]; int g_atrEMon[];
datetime g_lastShot[];     // [yenIdx*nHours + hourIdx]
datetime g_lastV4Bar[];    // [v4Idx]
datetime g_lastMonth[];    // [e5Idx]
datetime g_lastEMonShot[]; // [emonIdx]
double   g_initBal=0.0, g_weeklyRisk=2.5, g_v4risk=0.9, g_e5leg=1.5, g_emonWeekly=2.5, g_maxLossPct=10.0;
...
{
   g_weeklyRisk=InpWeeklyRiskPct; g_v4risk=InpV4RiskPerTradePct; g_e5leg=InpE5LegRiskPct; g_emonWeekly=InpEMonWeeklyPct;
   ...
   if(InpScenario==PORT_INSTANT){
      // 加重S1(25:35:15:25)・現状INSTANT(2.0x,maxDD-9.7%)とDDを合わせ収益+45%(docs/53)
      g_scenName="INSTANT_2.0x_W"; g_weeklyRisk=1.00; g_v4risk=0.36; g_e5leg=0.60; g_emonWeekly=1.00;
      g_useTrailing=true;
   } else if(InpScenario==PORT_PROP){
      // 加重S1・現状PROP(3.0x,maxDD≈-15%)とDDを合わせ収益+46%(docs/53)
...
int OnInit()
{
   if(!InpAcknowledgeLEAD){ Print("[STOP] v4=ADOPT/E5・E-Mon=LEAD。InpAcknowledgeLEAD=trueで承認(デモ/小サイズ)。"); return INIT_FAILED; }
   ...
   if(ny==0||nv==0||ne==0||nm==0||nh==0){ Print("シンボル/時刻のパース失敗"); return INIT_FAILED; }
   ResolveScenario();
   ...
   ArrayResize(g_atrMN1,ne); ArrayResize(g_atrEMon,nm);
   ...
   ArrayResize(g_lastEMonShot,nm); ArrayInitialize(g_lastEMonShot,0);
   ...
   for(int i=0;i<nm;i++){
      string r=ResolveSymbol(g_emon[i]); g_atrEMon[i]=INVALID_HANDLE;
      if(r==""){ PrintFormat("⚠ E-Mon銘柄 %s 見つからず→スキップ",g_emon[i]); continue; }
      if(r!=g_emon[i]) PrintFormat("[銘柄解決] E-Mon %s → %s",g_emon[i],r);
      g_emon[i]=r; g_atrEMon[i]=iATR(g_emon[i],PERIOD_H1,InpEMonAtrH1);
   }
   trade.SetDeviationInPoints(InpSlippagePoints);
   g_peakEquity=g_initBal; g_trailFloor=g_initBal*(1.0-g_maxLossPct/100.0);
   ResetDay(TimeCurrent());
   PrintFormat("[INIT P4 %s] initBal=%.0f v7=%.2f%% v4=%.2f%% E5=%.2f%% E-Mon=%.2f%% trailing=%s profit=%s(%.0f) daily=%s",
      g_scenName,g_initBal,g_weeklyRisk,g_v4risk,g_e5leg,g_emonWeekly,(g_useTrailing?"Y":"N"),
      (g_useProfitStop?"Y":"N"),g_profitPct,(g_useDailyStop?"Y":"N"));
   if(InpVerboseLog) Print("[NOTE] 1チャートに本EA1つ。v7+v4+E5+E-Monを内部運用。Magic分離(",g_mV7,"/",g_mV4,"/",g_mE5,"/",g_mEMon,")。E5/E-Mon=LEAD=デモ前提。合算DDをデモで実測し-10%枠内か確認。");
   EventSetTimer(30);
...
void EntriesV4()
{
   ...
   for(int i=0;i<ArraySize(g_v4);i++){
      ...
      if(sig>0){ double e=SymbolInfoDouble(sym,SYMBOL_ASK);
         ...
         if(trade.Buy(lots,sym,0.0,sl,tp,"v4_"+sym) && InpVerboseLog) PrintFormat("[v4 ENTRY] LONG %s lots=%.2f",sym,lots); }
      else     { double e=SymbolInfoDouble(sym,SYMBOL_BID);
         double sl=NormalizeDouble(e+sd,dg), tp=NormalizeDouble(e-tpd,dg);
         if(trade.Sell(lots,sym,0.0,sl,tp,"v4_"+sym) && InpVerboseLog) PrintFormat("[v4 ENTRY] SHORT %s lots=%.2f",sym,lots); }
   }
}
...
void EntriesEMon(datetime utc)
{
   ...
   if(u.hour!=InpEMonHourUTC) return;            // 指定時刻
   datetime hourBar=utc-(utc%3600);
   int nm=ArraySize(g_emon);
   double perShot=g_emonWeekly/(nm>0?nm:1);      // 週次予算を指数数で分割(合算建玉一定)
   trade.SetExpertMagicNumber(g_mEMon);
   for(int s=0;s<nm;s++){
      if(g_atrEMon[s]==INVALID_HANDLE) continue;
      if(g_lastEMonShot[s]==hourBar) continue;
      string sym=g_emon[s];
      double atr=AtrAt(g_atrEMon[s]); if(atr<=0) continue;
      double bid=SymbolInfoDouble(sym,SYMBOL_BID), ask=SymbolInfoDouble(sym,SYMBOL_ASK);
      if(ask<=0||bid<=0) continue;
      if((ask-bid)/MathMax(ask,1.0) > InpEMonMaxSpreadFrac){ g_lastEMonShot[s]=hourBar; continue; }
      double sd=InpEMonCatATR*atr;                // 保険SL(時間決済が主)
      double riskMoney=g_initBal*(perShot/100.0);
      double lots=LotsFor(sym,sd,riskMoney); if(lots<InpMinLot){ g_lastEMonShot[s]=hourBar; continue; }
      int dg=(int)SymbolInfoInteger(sym,SYMBOL_DIGITS);
      double sl=NormalizeDouble(ask-sd,dg);
      g_lastEMonShot[s]=hourBar;
      if(trade.Buy(lots,sym,0.0,sl,0.0,StringFormat("EMon_%s",sym)))
         { if(InpVerboseLog) PrintFormat("[E-Mon ENTRY] LONG %s lots=%.2f SL=%.2f",sym,lots,sl); }
   }
}
//+------------------------------------------------------------------+
//| 残存リスク(誠実な記録):                                          |
//|  ・v4=ADOPT(実10年Dukascopy 6/6)。E5・E-Mon=STRONG-LEAD(未確証)。  |
//|    いずれも本資金前にデモ前進検証必須(docs/29と同方針)。           |
//|  ・既定サイズ=「加重S1(25:35:15:25)・現状とDDを合わせた同DD版」     |
//|    (INSTANT 2.0x相当 / PROP 3.0x相当, docs/53)。同DDで年率+45〜65% |
//|    を狙うが、これは測定した低相関(0.2前後)が将来も保つ前提。       |
//|    危機時に相関が上がると合算DDが膨らむ → デモで実maxDDが−10%枠内  |
//|    かを必ず実測し、超えたらまず E-Mon を削る。                     |
//|  ・株価指数=配当抜き検証。CFD実スプレッド/オーバーナイトスワップ/  |
//|    配当/月曜の建て時刻は業者差大 → デモで実測。                    |
//|  ・4戦略Magic分離(base+1/+2/+3/+4)。v7/v4/E-Monは銘柄を一部共有する |
//|    がMagicで別管理(ネッティング口座では相殺に注意)。              |
//|  ・1チャートに本EAは1つだけ。等加重(S3)で使うなら PORT_MANUAL +    |
//|    weekly/v4/E5/EMon を等リスク配分に設定(docs/52)。              |
//+------------------------------------------------------------------+
