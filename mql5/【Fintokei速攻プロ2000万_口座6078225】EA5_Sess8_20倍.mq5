...
input string InpMonLegs  = "";                          // Mon: EA5 では空(既存 C6m EA が担当)
...
input datetime InpExpiry = D'2026.10.13 23:59';  // 速攻プロ60日枠(購入2026-08-15起点)
...
input double InpBalGuardPct      = 1.5;   // equity≤日開始balance−この%で全決済+当日停止(0=無効)
...
input long   InpMagicBase = 944400;  // Sess=+5(既存 C6m EA 943700 と別基底)
...
string  g_sesSym[MAXLEG]; bool g_sesShort[MAXLEG]; int g_sesH0[MAXLEG]; int g_sesSpan[MAXLEG]; double g_sesW[MAXLEG]; int g_nSes=0;   // EA5
...
int OnInit()
{
   ...
   { double ws2=0; for(int i=0;i<g_nSes;i++) ws2+=g_sesW[i];
     PrintFormat("[INIT Sess EA5] legs=%d Σw=%.3f mult=%.1f (グロス想定≈%.1fx・20-00UTC窓 同時≈%.1fx) spreadCap=%.1fpip minSL=%.0fpip Magic=%I64d",
        g_nSes,ws2,InpSessMult,ws2*InpSessMult,0.615*InpSessMult,InpSessMaxSpreadPips,InpSessMinStopPips,InpMagicBase+5); }
...
void EntriesSess(datetime utc)
{
   if(g_nSes==0) return;
   MqlDateTime u; TimeToStruct(utc,u);
   if(u.day_of_week<1 || u.day_of_week>4) return;          // 月〜木
   if(HolidayBlocked(utc)) return;
   { string today=StringFormat("%04d.%02d.%02d",u.year,u.mon,u.day);
     if(StringLen(InpSessSkipDates)>0 && StringFind(InpSessSkipDates,today)>=0) return; }   // FOMC 等
   ...
   for(int s=0;s<g_nSes;s++){
      ...
      if(CountPos(sym,g_mSes)>0) continue;
