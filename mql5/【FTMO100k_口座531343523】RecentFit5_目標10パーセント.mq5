//+------------------------------------------------------------------+
//| ★FTMO#531343523専用焼き込み版 v1.02(docs/178+S5再試行修正) |
//|  基準残高100000(フェーズ開始値・自動検出は使わない)                |
//|  ロック10.0/10.5(FTMO P1目標+10%実査確定)・通知ON                  |
//|  v1.01: 族C(S4/S5)のバー内再試行を追加(スプレッド/市場閉スキップが |
//|         当日恒久化するバグの修正・2026-08-03 S5事象)               |
//|  v1.02: 再試行の断念判定を回数→時間(初回失敗+1時間・30秒間隔)に   |
//|         変更(ティック駆動での早すぎる断念を防止)/族C決済の保有    |
//|         バー数をiBarShift化(ライブで1日長い保有を修正)/日次ガード |
//|         基準の再起動復元                                          |
//|  P2移行時: InpLockArmPct=5.0 / InpLockClosePct=5.5 / BaselineReset |
...
input double InpInitialBalance    = 100000.0;    // 0=自動(初回アタッチ時残高を端末に永続保存)
...
input double InpLockArmPct        = 10.0;    // equity+この%で新規停止(P1=8.0 / P2=5.0)
input double InpLockClosePct      = 10.5;    // equity+この%で全決済・恒久ロック(P1=8.5 / P2=5.5)
...
input bool   InpNotifyEnable      = true;  // SendNotification(要MetaQuotes ID)
