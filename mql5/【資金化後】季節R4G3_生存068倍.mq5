//+------------------------------------------------------------------+
//|                【資金化後】季節R4G3_生存068倍.mq5                  |
//|   第3ポートフォリオ「生存モード」(docs/128): 資金化した口座に        |
//|   挿すだけの R4+G3 @0.68x(ガード安全倍率)。                        |
//|                                                                   |
//|   ★使い方: 資金化口座のチャートにドロップ → OK。                   |
//|     切替時は InpBaselineReset=true で基準残高を今の残高で取り直す。 |
//|   ★校正(docs/110 パネルA・日次10.5年):                             |
//|     0.68x = 平均月利+1.13% / 年率+14.2% / Sharpe 2.21 /            |
//|     maxDD−2.9% / 最悪単日−2.4% / 10.5年でガード非接触。            |
//|   ★チャレンジ版との違い(既定値のみ・ロジック同一):                  |
//|     倍率1.5→0.68 / 利益ロックOFF(資金化後は通過目標なし・           |
//|     出金はdocs/49の定石で手動)。ガード日次−4%/フロア−8%は維持。     |
//|                                                                   |
//|   ⚠ デモ必須: R4はLOYO合格(docs/105)だがデモ未了。                  |
//|     1口座目の資金化は pd_funded_conserv(Drive確定)が既定。          |
//|     本EAはデモ採点(docs/103)で分布内を確認後の2口座目以降(docs/128)。|
//|   ⚠ 実装は Chien_Seasonal_R4G3_Prop v1.30 の逐語コピー(既定値のみ   |
//|     変更・Magic基底940700同一=同口座でチャレンジ版と併用しない)。   |
//+------------------------------------------------------------------+
#property copyright "chien-monitor research"
#property version   "1.30"   // = Seasonal_Top_EA v1.50 相当
#property strict
#property description "[FUNDED survival 0.68x] R4+G3 ONE-CLICK for funded accounts (guard-safe mult, profit-lock OFF). Drop on chart & OK. Demo-first (docs/128)."
...
input double InpMultOverride    = 0.68;   // ガード安全倍率(docs/110 パネルA)。チャレンジ用は1.5
...
input bool   InpProfitLockEnable  = false;// 資金化後は通過目標なし=既定OFF(出金は手動, docs/49)
