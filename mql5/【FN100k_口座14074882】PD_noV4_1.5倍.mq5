//+------------------------------------------------------------------+
//|      【FN100k 口座14074882】PD_noV4 1.5倍 (v1.44・docs/171)       |
//|  構成: v7(EURJPY+GBPJPY) 35.7 / E-Mon[RG3] 35.7 / E5(XAU+NAS) 28.6 |
//|  v4除外(危機日の集中テール切除)・balance基準ガード-3.5%搭載        |
//|   ワンクリック版: チャートにドロップ→OKだけ(設定・.set不要)         |
//|                                                                   |
//|   構成(docs/134-135・間引き確定版):                                |
//|     v7 = EURJPY+GBPJPY(USDJPY除外・Dukascopy合格)                  |
//|     E5 = XAUUSD+NAS100(US500/GER40除外・26年耐久合格)              |
//|     v4/E-Mon = 現行のまま(v4間引きは実データで見送り)              |
//|   サイズ: 3倍相当×間引き倍率比0.951(残り目標を守りながら取る)      |
//|   仕上げロック: ARM+7.9%(新規停止)→+8.05%全決済=P1通過確定         |
//|     ※現在のequityがどの水準でも機能(旧版のARM7.0/LOCK7.5から変更)  |
//|   防御(v1.31): 休日窓12/20-1/3新規停止・ペア別スプレッド上限        |
//|   基準残高: $100,000 焼き込み(この口座専用)                        |
//|                                                                   |
//|   ⚠ 間引き版はデモ未了のまま本口座適用(2026-07-13ユーザー判断)。    |
...
input double InpNotifyRefPct  = 7.8;   // 手決済の参考値: equity+この%で「検討ライン」通知
...
input double InpWeeklyRiskPct    = 0.78;  // v7 週次リスク%(noV4 35.7% × 1.5x)
input double InpV4RiskPerTradePct= 0.00;  // v4 無効(noV4・docs/171)
input double InpEMonWeeklyPct    = 0.78;  // E-Mon 週次リスク%(noV4 35.7% × 1.5x・RG3 ON)
input double InpE5LegRiskPct     = 1.25;  // E5 legRisk%(noV4 28.6% × 1.5x)
input double InpProfitStopPct = 8.1;   // +この%で新規停止(FN P1=8.0)
...
input long   InpMagicBase = 761030;  // v7=+1/v4=+2/E5=+3/E-Mon=+4(noV4版専用基底)
