...
#property version   "1.20"   // docs/276: EA7 Mon 4 + Roll 5(Sess スリーブ再利用・曜日指定・金利差門)
#property strict
#property description "[EA7 Fintokei Swing] Mon JPY-cross 4 x2.0 + Roll 5 x3.0 (Wed 20:00 UTC short, 4h, carry gate). Swap-free account only. Balance guard -2.8 tick, floor -9, P1 lock 8.05 (P2 6.05)."
...
input string InpMonLegs  = "GBPJPY:0.260,EURJPY:0.266,AUDJPY:0.215,USDJPY:0.258"; // Mon: 月曜o2o LONG(docs/276)
input string InpV4Legs   = "";                          // v4: 未使用
input string InpHoldLegs = "";                          // Hold: 未使用(スワップ有り口座)
input double InpMult     = 2.0;   // Mon スリーブ倍率(docs/276 §2: 3% 警告の内側)

input group "=== 有効期限(直近特化=賞味期限つき。docs/174停止規則) ==="
input datetime InpExpiry = D'2027.03.31 23:59';  // 再校正日(スイングに期限は無い。docs/276)
...
input double InpBalGuardPct      = 2.8;   // equity≤日開始balance−この%で全決済+当日停止(Fintokei 同時保有リスク 3% 警告の内側)
...
input double InpSessMult          = 3.0;   // Roll 専用倍率(名目=基準残高×重み×倍率。上限 3.0・docs/273 §1)
...
input string InpSessSkipDates     = "";    // Roll を建てない日(UTC・空=無し。docs/265: FOMC 日も同じ挙動)
