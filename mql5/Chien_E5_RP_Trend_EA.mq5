...
//|   ★ステータス: STRONG-LEAD（検証済みエッジではない）。本資金禁止＝   |
//|     デモ/極小サイズの衛星でのみ運用し、正の期待値とv7低相関(−0.15)が |
//|     フォワードで再現するかを確認する目的。docs/31でv7同等7ゲート中    |
//|     5/5の頑健性(WF5/5・全変種+・プラセボ崩壊)を確認、未達は有意水準    |
//|     2つのみ。詳細 docs/27, docs/29, docs/30, docs/31。               |
//|                                                                   |
...
//|       g_legRisk% of equity」に揃える(=低ボラ資産ほど   |
...
enum ENUM_E5_SCENARIO
{
   E5_INSTANT_FUNDED_75_25 = 0, // 運用(インスタント/資金化後) 75:25 → legRisk 0.46
   E5_PROP_BREAKTHROUGH_65_35 = 1, // プロップ突破 65:35 → legRisk 1.23
   E5_MANUAL = 2                // 下のInpLegMonthlyRiskPctを使用
};
...
input double InpLegMonthlyRiskPct = 0.55;   // (MANUAL時のみ)1レッグの月次P&L目標σ%
...
CSymbolInfo   sym;
double   g_initBal = 0.0;   // 実効口座サイズ(0入力で自動)
double   g_legRisk = 0.55;  // 実効legRisk(シナリオで自動設定)
...
int OnInit()
{
   ...
   // シナリオでlegRiskを自動設定(MANUALならInput値)
   if(InpScenario==E5_INSTANT_FUNDED_75_25)      g_legRisk=0.46;
   else if(InpScenario==E5_PROP_BREAKTHROUGH_65_35) g_legRisk=1.23;
   else g_legRisk=InpLegMonthlyRiskPct;
   ...
   PrintFormat("[INIT E5/%s] LEAD demo satellite. legRisk=%.2f%%/月 floorDD=%.1f%% LB=%d/%d/%d/%d short=%s",
               _Symbol,g_legRisk,InpAccountFloorDDPct,InpLB1,InpLB2,InpLB3,InpLB4,
               (InpAllowShort?"Y":"N"));
   if(InpVerboseLog)
...
void OpenDir(int dir)
{
   ...
   if(ok) PrintFormat("[ENTRY E5/%s] %s lots=%.2f SL=%.5f atrMN1=%.5f legRisk=%.2f%%",
                      _Symbol,(dir>0?"LONG":"SHORT"),lots,sl,atr,g_legRisk);
