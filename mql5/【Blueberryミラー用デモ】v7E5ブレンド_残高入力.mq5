...
#property strict
#property description "[BLUEBERRY-MIRROR demo v7+E5 65:35 / lots based on InpInitialBalance] Portfolio-D ONE-CLICK (PROP median-3 standard): v4+v7+E-Mon[RG3]+E5=30:25:25:20. Drop on chart & OK. v1.10 profit lock+risk guard+swap log / v1.20 push notify (Even G2)+persistent baseline+E5 re-enter (docs/112) / v1.30 month-trail lock T2 (docs/116, default OFF)."
...
input group "=== 口座/ガード ==="
input double InpInitialBalance   = 50000.0;   // 0=自動(初回アタッチ時の残高を端末に永続保存・再アタッチ耐性)
...
input bool   InpProfitLockEnable = false;  // 利益ロックを使う
...
void EntriesE5()
{
   ...
   for(int i=0;i<ArraySize(g_e5);i++){
      ...
      double equity=g_initBal;   // ミラー先残高基準(デモ残高に依存しない)
