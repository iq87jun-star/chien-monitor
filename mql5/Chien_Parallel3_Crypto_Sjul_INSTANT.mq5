//+------------------------------------------------------------------+
//|                 Chien_Parallel3_Crypto_Sjul_INSTANT.mq5         |
//|   第3並行ポートフォリオ・オールインワン(インスタント既定):         |
...
//|                                                                   |
//|   ■ 本ファイル=インスタント既定(Blueberry等・即時資金化):           |
//|     ★CRYPTOは高DDゆえ小サイズ既定。攻め=CRYPTO週次0.80%/S-Jul月次     |
//|     0.60%・−10%トレーリング/無目標・全停止フロア−9%。                |
//|     守りは P3_MANUAL で週次0.40/月次0.30・フロア−8%(プリセット同梱)。 |
//|     (プロップ審査運用は Chien_Parallel3_Crypto_Sjul_PROP.mq5)        |
//|                                                                   |
//|   ■ 内部2戦略(Magic分離で混線なし・Magic帯=950750):                 |
...
#property strict
#property description "3rd parallel portfolio all-in-one (INSTANT default): CRYPTO(BTC/ETH Monday LONG)+S-Jul(equity-index July seasonal LONG). Per-strategy magics. CRYPTO=significant/high-DD, S-Jul=SEASONAL-LEAD: demo first."
...
input ENUM_P3_SCENARIO InpScenario = P3_INSTANT; // ★本ファイル既定=インスタント攻め
...
input long   InpMagicBase = 950750;       // CRYPTO=+1 / S-Jul=+2 (PROP版950740・既存950710/720/730と非衝突)
...
//|    ・本第3ポートPROP版(950740)とは別Magic帯(950750)で衝突しない。   |
