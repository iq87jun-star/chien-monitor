//+------------------------------------------------------------------+
//|                    Chien_Portfolio2_AllInOne_PROP.mq5            |
//|   第2(並走)ポートフォリオ オールインワン(プロップ既定)。         |
...
//|                                                                   |
//|   ■ 本ファイル=プロップ既定 ★突破攻め(FundedNext/FTMO等):           |
//|     MR 0.75%/トレード / RP 1.50% legRisk・静的−10%/+8%停止/日次−4%。 |
//|     全停止フロア=−9%(枠を使い切る攻め)。気長確実は PORT_MANUAL 1.5x。|
//|     ⚠突破攻め=業者破綻/規約変更リスク前提=速く突破し資金化後も攻めて |
//|       早期に出金・口座に大金を置かない。守りは資金化後に PORT_MANUAL。|
//|     (インスタント運用は Chien_Portfolio2_AllInOne_INSTANT.mq5 を使う) |
//|                                                                   |
//|   ■ 内部2戦略(Magic分離で混線なし):                                 |
...
#property strict
#property description "Parallel(2nd) portfolio all-in-one (PROP default): MR(v4 mechanism on disjoint crosses)+RP(E5 mechanism on disjoint basket). No v7. Per-strategy magics. Validate/correlation via notebooks/portfolio2_validate."
...
input ENUM_PORT2_SCENARIO InpScenario = PORT_PROP_BREAKTHROUGH; // ★本ファイル既定=プロップ突破
...
input double InpMRRiskPerTradePct= 0.75;  // MR 1トレードあたりリスク%(PROP突破既定と同値)
input double InpRPLegRiskPct     = 1.50;  // RP legRisk%(各レッグ月次σ・PROP突破既定と同値)
...
input long   InpMagicBase = 940740;       // MR=+1 / RP=+2 (現行Portfolio/Portfolio3/P2-INSTANTと非衝突)
