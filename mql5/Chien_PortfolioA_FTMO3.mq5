...
//|     指数月曜(E-Mon)を持たない v4主軸=FN側との重複を減らした別EA。    |
//|     構成は docs/70 の twobook_A(v4core)と同一。                     |
//|                                                                   |
...
//|     残高自動・銘柄自動解決。Magic: base+1=v7 / +2=v4 / +3=E5         |
//|     (base=770720。FN側EAと別口座なら衝突なし)。                      |
//|                                                                   |
//|   ⚠ 本資金前にデモ必須。失格率は月次解像度=楽観側。                 |
...
string   g_yen[]; string g_v4[]; string g_e5[];
int      g_v7hours[];
...
int OnInit()
{
   ...
   if(ny==0||nv==0||ne==0||nh7==0){ Print("シンボル/時刻のパース失敗"); return INIT_FAILED; }
   ResolveScenario();
...
//+------------------------------------------------------------------+
//| 残存リスク(誠実な記録):                                          |
//|  ・失格率は月次解像度のMC=楽観側(FTMO日次−5%/月内DD/                |
//|    ギャップ未反映)。実際はこれより高い前提で。                     |
//|  ・サイズ既定は twobook_A(FN確定)×(+10%再校正比) の一次校正。       |
//|    正確な配分は notebooks/portfolio_D_deploy(Drive)+デモ実DDで確定。|
//|  ・v4とv7は円クロスを共有。Magic分離で別管理                       |
