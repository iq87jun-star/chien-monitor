//| ⚠ 2026-09-18 docs/249: Sess(時間帯)族は研究コードのコスト符号バグによる偽エッジと判明。Sess レグは使用禁止(EA2 は InpSessEnable=false で Mon のみ継続可)。 |
//+------------------------------------------------------------------+
...
//|  ★v1.11(2026-09-14): 既存 4 レグを既定で空にし、Sess 上位 5 のみ  |
//|    を回す版。既存建玉(Magic 943601〜943603)は本 EA では管理しない  |
//|    ため、切替前に手決済するか、旧 EA で決済してから付け替える。    |
//|  ★v1.10(2026-09-14): 時間帯セル Sess を追加(docs/233/234/235)。   |
//|    5年勝率上位5 = EURGBP 20-00S / NZDUSD 20-00S / AUDUSD 20-00S /  |
//|    EURGBP 16-20S / USDCHF 00-04S(月〜木・4h・SHORT・逆ボラ加重)。  |
//|    倍率 InpSessMult=4.8(docs/235 校正値=上限。docs/228 の推奨 3.3  |
//|    を超える試験運用。ユーザー判断 2026-09-14)。Magic=基底+5。      |
//|    既存 4 レグ(Mon/v4/Hold・1.48倍)とは独立にサイズ計算。          |
//|    建て時スプレッド上限 InpSessMaxSpreadPips(超過は見送り=コスト   |
//|    3pip 超で消える手法のため)。SL は最低 InpSessMinStopPips。      |
...
#property version   "1.11"   // Sess 上位5 のみ(既存 Mon/v4/Hold レグは既定で空。docs/236 §5)
#property strict
...
input string InpMonLegs  = "";                          // Mon: v1.11 では空(Sess 専用運用)
input string InpV4Legs   = "";                          // v4: 空
input string InpHoldLegs = "";                          // Hold: 空
...
input bool   InpHoldEnable    = false;  // v1.11: 既定 false(Sess 専用)
...
input string InpSessLegs          = "EURGBP:S:20:4:0.349,NZDUSD:S:20:4:0.143,AUDUSD:S:20:4:0.171,EURGBP:S:16:4:0.173,USDCHF:S:0:4:0.163"; // SYM:方向:建てUTC時:保有h:重み
input double InpSessMult          = 4.8;   // Sess 専用倍率(名目=基準残高×重み×倍率)
...
int OnInit()
{
   ...
   { int orphan=0;
     for(int i=PositionsTotal()-1;i>=0;i--){ ulong tk=PositionGetTicket(i); if(tk==0||!posinfo.SelectByTicket(tk)) continue;
        long mg=posinfo.Magic(); if((mg==g_mMon&&g_nMon==0)||(mg==g_mV4&&g_nV4==0)||(mg==g_mHold&&g_nHold==0)) orphan++; }
     if(orphan>0){ PrintFormat("⚠ [v1.11] 旧レグ(Mon/v4/Hold)の建玉 %d 件が残っていますが、本 EA は管理しません(SL更新・時間決済なし)。手決済するか旧 EA で決済してください。",orphan);
                   Notify(StringFormat("旧レグ建玉 %d 件が未管理",orphan)); FlushNotify(); } }
