...
export async function aggregate() {
  ...
  // --- 通貨: Exalted Orb基準のレートに正規化 ---
  const nameById = new Map(
    [...(currencyRaw.core?.items ?? []), ...(currencyRaw.items ?? [])].map((i) => [i.id, i]),
  );
  const lines = currencyRaw.lines ?? [];
  const exalted = lines.find((l) => l.id === "exalted");
  const base = exalted?.primaryValue || 1;
  ...
  const currencies = lines
    ...
    .map((l) => ({
      ...
      // Exalted Orb 1個に対するレート
      rateInExalted: l.primaryValue / base,
      change7d: round1(l.sparkline?.totalChange ?? 0),
      spark: l.sparkline?.data ?? [],
    }))
    .sort((a, b) => b.rateInExalted - a.rateInExalted);
  ...
  for (const type of UNIQUE_TYPES) {
    ...
    for (const l of data?.lines ?? []) {
      if (typeof l.primaryValue !== "number") continue;
      allUniques.push({
        name: l.name,
        baseType: l.baseType,
        category: type.replace(/^Unique/, ""),
        value: l.primaryValue,
