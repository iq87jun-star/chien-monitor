...
const SYSTEM_PROMPT = `あなたはPath of Exile 2(PoE2)の日本語市場アナリストです。渡された集計データだけを根拠に、日本語の解説記事を書きます。
...
- 読者は日本語圏のPoE2プレイヤー。専門用語(ex=Exalted Orb, div=Divine Orb等)は初出で軽く補足する。
- 文体は丁寧すぎない「です・ます」調。`;
...
export async function generateArticles() {
  ...
  const input = {
    ...
    topCurrencies: economy.currencies.slice(0, 10).map((c) => ({
      name: c.name,
      rateInExalted: Math.round(c.rateInExalted * 100) / 100,
  ...
  const userPrompt = `以下はPoE2「${input.league}」リーグの最新集計データです(通貨レートはExalted Orb基準、変動率は直近7日間の%)。
