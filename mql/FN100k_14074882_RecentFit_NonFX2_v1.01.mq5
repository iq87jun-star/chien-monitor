...
//|  v1.01 (2026-08-15): v1.00の検証結果を反映して最適化               |
//|    検証: research/nonfx_config_verification.md                     |
//|          research/verify_nonfx_config.py (再現可能)               |
//|                                                                   |
//|  構成(5セル・Σw=1.000): US500火L 0.333 / HK50月L 0.225 /          |
//|    HK50木S 0.225 / JP225水L 0.152 / XAG火L 0.065                  |
//|    ※v1.00の XPTUSD火L(0.072)は除外。逆ボラ規則で再正規化。         |
//|                                                                   |
//|  校正(0.8×規則・悲観=窓外系列基準・SL込みで再計算):                |
//|    窓外(選抜窓を除く約9年) 最悪日-1.55% / maxDD-8.47% → mult 0.72  |
//|    倍率後: 窓外maxDD-6.09%(フロア-8%に余裕) / 窓外年率+2.14% /     |
//|            選抜窓12M +14.3%                                        |
//|    ⚠v1.00の mult 1.0 は窓外maxDD-9.32%でフロアを突破していた。      |
//|                                                                   |
//|  v1.00からの主な変更(すべて検証根拠つき):                          |
//|   ①XPTUSDレッグ除外 — t=1.66(最弱)/損益分岐スプレッド$0.70に対し   |
//|     実勢$1.5-3.0で恒常赤字/元データPL=Fの O=H=L=C 退化バーが       |
//|     当該セル取引の79%(選抜窓内75%)で成績そのものが信用できない     |
//|   ②mult 1.0 → 0.72 (EA自身の校正規則が出す値に戻す)               |
//|   ③災害SL: 2.5×ATR(H1,24) → 5.0×ATR(D1,14)                       |
//|     旧: SL距離≒日次レンジ0.5倍で接触率42-59%。研究セル(SLなし)と   |
//|         別戦略になり12M合成 +20.8%→+12.2% に劣化していた。         |
//|     新: SL距離≒価格の7.5%。10年で接触率0.0%(成績への影響ゼロ)     |
//|         かつ単発損失を equity の1.8%に限定する真の災害SL。         |
...
#property strict
#property description "[RecentFit NonFX2 FN100k 14074882 v1.01] 5 legs: SPX500-Tue-L .333 / HK50-Mon-L .225 / HK50-Thu-S .225 / JP225-Wed-L .152 / XAG-Tue-L .065."
#property description "Per-leg entry hours (parity with each cash open). mult 0.72 (0.8x rule on out-of-window window)."
#property description "Disaster SL 5.0*ATR(D1,14). Floor -8 (tick), daily -3 stop / -4 close (server day, max(bal,eq)). Lock off (funded). Expiry 2026-10-15 UTC."
...
//   XAGUSD CME寄り≒前日23:00UTC → 0|1|2|3
...
input double InpMult     = 0.72;   // リスク倍率(窓外0.8x規則=0.72。12M窓パリティ上限は3.64)
...
input double InpCatastropheATR= 5.0;    // 災害SL=5.0×ATR(D1,14)≒価格の7.5%(10年で接触率0.0%)
...
int OnInit()
{
   ...
   // v1.01③: 災害SLは日足ATRベース(時間足ATRでは"災害"にならない)
   ...
   PrintFormat("[INIT RF-NonFX2 v1.01] initBal=%.0f mult=%.2f Σw=%.3f (グロス想定≈%.2fx) floor=-%.1f%% daily=-%.1f/-%.1f%% lock=%s expiry=%s Magic=%I64d/%I64d/%I64d",
   ...
   Print("[NOTE] 直近特化トラック(docs/174)。窓外期待値は年率+2.14%(mult0.72・コスト未計上)。");
...
void EntriesDow(datetime utc)
{
   ...
   for(int s=0;s<g_nDow;s++){
      ...
      double sd=InpCatastropheATR*atr; double sp=sd/pip;   // v1.01③: 災害SL=5.0×ATR(D1,14)
      ...
      if(spbp>SpreadCapBpFor(s)) continue;
...
//|    窓外(選抜窓を除く約9年)の実力は年率+2.14%(mult0.72・コスト前)。 |
...
//|  ・災害SLは10年で接触率0.0%=歴史的には一度も発動していない。       |
//|    発動時の単発損失は equity の約1.8%(最大重みレッグ)。            |
