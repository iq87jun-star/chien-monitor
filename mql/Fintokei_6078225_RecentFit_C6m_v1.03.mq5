...
//|                                                                   |
//|  v1.03 (2026-08-14 コードレビュー反映):                            |
//|   ④BAL_GUARD(-1.5%全決済)のアンカーを max(日開始balance, 日開始    |
//|     equity) に変更。Fintokei日次は有効証拠金(equity)のUTC0再計算   |
//|     のため、含み益で日をまたぐ日(Mon24h保有/v4最大8日保有で常態)は  |
//|     balance基準だと全決済が規約-2%より後に来る経路があった。       |
//|   ⑤DAY_PROFIT_CAP(+2.5%新規停止)の当日停止をGV永続化。損失側(_ds)  |
//|     と同様に、当日中の再起動で停止が解除されるのを防ぐ。           |
//|   ⑥日次+2.9%で全決済(InpDayProfitClosePct)を追加。新規停止だけでは |
//|     既存建玉(グロス想定2.5倍)の伸びで日次+3%を超え得るため、       |
//|     決済側でも当日利益を規則の内側に確定させる。                   |
//|   ⑦PASS_LOCK決済閾値 6.05→6.15(目標6%との余裕0.05%=2,000万口座で   |
//|     1万円は決済スリッページで割り込み得るため)。保険停止6.1→6.2。  |
//|   ⑧有効期限判定を TimeCurrent(サーバー時刻)→TimeGMT(UTC)に統一。   |
//|     InpExpiry はUTCとして解釈される。                              |
//|                                                                   |
//|  系譜: Chien_RecentFit_2026H2_Prop(docs/174設計/175計測)のC6m派生。 |
//|   直近特化=事前固定ルール選抜のEVベット(耐久エッジの主張はしない)。 |
//|   標準版の構成(v4 USDJPY/Hold JP225/倍率4.8/7.2)とは異なる点に注意。|
//|   構成凍結: recentfit_screen.py 2026-07-30(docs/175)。             |
//|   停止規則(docs/174): 期限(2026-10-13)超過で新規停止=再スクリー    |
//|   ニング必須。チャレンジ失格2回でトラック撤退(費用予算の上限)。    |
...
#property strict
#property description "[RF C6m 2.5x] Fintokei SwiftTrader 20M #6078225 (docs/183). Mon GBPJPY+AUDJPY / v4 NZDUSD+AUDUSD. mult 2.5, lock 5.9/6.15, floor -2.4, daily -1.2 stop / -1.5 close (UTC0 max(bal,eq) anchor), day profit +2.5 stop / +2.9 close. Expiry 2026-10-13 UTC."
...
input datetime InpExpiry = D'2026.10.13 23:59';  // 速攻プロ60日枠(購入2026-08-15起点・UTC解釈)

input group "=== 口座/ガード ==="
input double InpInitialBalance   = 0.0;   // 0=自動(初回アタッチ時の残高を端末に永続保存)
input bool   InpBaselineReset    = false; // 新フェーズ開始時のみtrue=基準残高を取り直す
input double InpMaxLossLimitPct  = 3.0;   // 失格ライン%(速攻プロ=初期残高静的-3%)
input double InpAccountFloorDDPct= 2.4;   // 全停止ライン%(-3%枠の0.8倍手前)
...
input double InpDayProfitClosePct= 2.9;   // v1.03: 日次equity+この%で全決済(利益上限3%の決済側防御。0=無効)

input group "=== v1.44 balance基準日次ガード(docs/170/171) ==="
input double InpBalGuardPct      = 1.5;   // equity≤日次アンカー(max(日開始bal,eq))−この%で全決済+当日停止(0=無効)
input int    InpBalGuardMaxMonth = 2;     // 月内発動上限(超過は月末まで新規停止)

input group "=== 利益ロック(目標6%土俵。Arm=新規停止/Close=全決済で通過確定) ==="
...
input long   InpMagicBase = 943700;  // Mon=+1/v4=+2/Hold=+3
...
datetime g_profitCloseDay=0;                       // v1.03: 日次利益全決済の当日一回制御
...
void BalGuardCheck()
{
   ...
   // v1.03: Fintokei日次-2%は有効証拠金(equity)のUTC0再計算。含み益で日を
   // またいだ日は日開始equity>balanceとなり、balance基準では全決済が規約
   // ラインより深い位置に来るため、高い方をアンカーに採る(常に保守側)。
   ...
   PrintFormat("[BAL GUARD] eq %.2f <= 日次アンカー %.2f (日開始bal %.2f/eq %.2f) -%.1f%% → 全決済・当日停止(月内%d回目%s)",
               beq,anchor,g_dayStartBal,g_dayStartEq,InpBalGuardPct,g_balFires,(g_balMonthHalt?"・月末まで停止":""));
...
void OnTimer()
{
   ...
   // v1.03: 判定をUTCに統一(他のUTC0基準ガードと整合。InpExpiryはUTC解釈)。
   ...
   // v1.03: 利益キャップをInpDailyStopPctから独立させ、当日停止をGV永続化
   // (旧実装は再起動でg_dayBlockedが失われ、equityがキャップ未満へ戻ると新規再開した)
   if(InpDayProfitCapPct>0 && dpnl>=g_initBal*InpDayProfitCapPct/100.0 && !g_dayBlocked){ g_dayBlocked=true;
      if(g_gvName!="") GlobalVariableSet(g_gvName+"_ds",(double)(long)g_curDay);
      PrintFormat("[DAY PROFIT CAP] %+.2f → 当日新規停止(1日利益上限3%%規則の手前)",dpnl);
      Notify("DAY_PROFIT_CAP 当日新規停止"); FlushNotify(); }
   // v1.03: 決済側の利益キャップ。新規停止だけでは既存建玉(グロス想定2.5倍)の
   // 伸びで日次+3%を超え得るため、+2.9%で全決済し当日利益を規則の内側に確定。
   if(InpDayProfitClosePct>0 && g_profitCloseDay!=g_curDay
      && dpnl>=g_initBal*InpDayProfitClosePct/100.0){
      g_profitCloseDay=g_curDay; g_dayBlocked=true;
      if(g_gvName!="") GlobalVariableSet(g_gvName+"_ds",(double)(long)g_curDay);
      CloseAllMine("DAY_PROFIT_CLOSE");
      PrintFormat("[DAY PROFIT CLOSE] %+.2f → 全決済・当日新規停止(利益上限3%%の決済側防御)",dpnl);
      Notify(StringFormat("DAY_PROFIT_CLOSE +%.1f%% 全決済・当日停止",InpDayProfitClosePct)); FlushNotify(); }
...
//|    ガード類はリスクベースEA(v1.44)と同一(+v1.03のアンカー変更)。  |
//|  ・日次ガードはティック評価だが週末ギャップ/急変時のスリップは    |
//|    防げない(docs/169クラッシュ監査参照)。窓開けが日次+3%と        |
//|    PASS_LOCKを同時に跳び越えた場合、決済実現益が1日利益上限3%を   |
//|    超え得る(決済側キャップ+2.9%でも防げない=残存リスク)。         |
//|  ・(本焼き込みではHold不使用) JP225 Holdは配当調整・スワップが    |
//|    業者差大=デモで実測すること。                                  |
//+------------------------------------------------------------------+
