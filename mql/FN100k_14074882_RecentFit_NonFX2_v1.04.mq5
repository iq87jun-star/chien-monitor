...
//|  v1.04 (2026-08-16): フロア-9%版 — 実機equity判明による再校正      |
//|    初回アタッチログで口座が 95,817 (-4.18%、季節RG3時代の損失) から |
//|    出発することが判明。P1目標まで+12.71%/旧フロア-8%まで3.98%。    |
//|    実出発点の再MC: mult2.5/フロア-8% → P1到達38.9%/失格53.2%       |
//|                    mult2.5/フロア-9% → P1到達43.1%/失格45.2%       |
//|    ユーザー決定 2026-08-16「フロア-9%で続行」。                     |
//|    ⚠フロア-9%は静的-10%への余裕が1%のみ。ギャップ時は自主停止では  |
//|      なく法的失格に直行し得る(承知の上の選択)。                    |
//|    ⚠悪い1日(歴史最悪日x2.5=-3.89%)がフロア余裕(4.98%)に迫る。      |
//|    根拠: research/nonfx2_restart_mc.py                              |
//|                                                                   |
//|  v1.03 (2026-08-15): チャレンジ校正版 — mult 2.5 / 利益ロック有効  |
//|    ユーザー確認により本口座は「チャレンジ段階」(funded想定を撤回)。 |
//|    目的関数が「生存」から「P1 +8%到達」に変わるため、mult を       |
//|    窓外0.8x規則(0.90)から 12M窓系校正レンジの 2.5 に引き上げた。   |
//|    根拠: research/nonfx2_challenge_mult.py (保守系列・20,000パス)   |
//|      mult 2.5 → P1通過64.3% / 失格26.9% / 中央値78営業日 /         |
//|                 資金化(P1xP2)47.8%。通過率は2.5で頭打ち、3.5は     |
//|                 歴史最悪日x3.5=-5.44%で日次-5%規則に抵触。          |
//|    ⚠これは docs/174 流のEVベット(失格27%を受け入れて資金化48%を    |
//|      買う)。ユーザー決定 2026-08-15「2.5でお願いします」。          |
//|    ⚠資金化した時点で mult を 0.90 に戻すこと(全歴史適合は0.90のみ、|
//|      §10-1)。P2移行時は Lock 4.9/5.05/5.1 + InpBaselineReset=true。|
//|                                                                   |
...
#property strict
#property description "[RecentFit NonFX2 FN100k 14074882 v1.04] CHALLENGE calibration: mult 2.5, floor -9, profit lock ON (P1 7.9/8.05/8.1)."
#property description "From actual start -4.18%: P1 pass 43.1% / bust 45.2% / median 111 trading days (conservative, 20k paths). Revert to mult 0.90 + floor -8 once funded."
...
#property description "Per-leg entry hours (parity with each cash open)."
#property description "Stop 0.5*ATR(D1,14) ~= 2.5*ATR(H1,24); fires ~31% so it is part of the strategy, not a disaster stop."
#property description "Floor -9 (tick, 1% before static -10), daily -3 stop / -4 close (server day, max(bal,eq)). Expiry 2026-10-15 UTC."
...
input double InpMult     = 2.50;   // チャレンジ校正(P1通過64%/失格27%)。資金化後は0.90へ戻す
...
input double InpAccountFloorDDPct= 9.0;   // v1.04: -10%枠の1%手前(実出発-4.18%でのP1到達43%を優先)
...
input bool   InpProfitLockEnable = true;   // v1.03: チャレンジ=有効(P1 7.9/8.05/8.1)
...
int OnInit()
{
   ...
   PrintFormat("[INIT RF-NonFX2 v1.04] initBal=%.0f mult=%.2f Σw=%.3f (グロス想定≈%.2fx) floor=-%.1f%% daily=-%.1f/-%.1f%% lock=%s expiry=%s Magic=%I64d/%I64d/%I64d",
   ...
   Print("[NOTE] チャレンジ校正 mult2.5/フロア-9%: 実出発-4.18%からP1到達43%/失格45%/中央値111営日。資金化後はv1.02(0.90/-8%)へ。");
...
void OnTimer()
{
   ...
   Comment(StringFormat("FN14074882_RF_NonFX2 v1.04 | gain %+.2f%% | mult %.2f | %s",gainPct,InpMult,
