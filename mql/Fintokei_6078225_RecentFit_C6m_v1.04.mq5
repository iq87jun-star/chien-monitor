...
//|   RecentFit C6m v1.04 — 倍率2.5→4.0 (2026-08-18 ユーザー決定)      |
//|                                                                   |
//|  ★v1.04変更 (InpMult のみ・ロジック不変):                          |
//|   速攻プロでは通過以外の結末 (日次失格/フロア停止/期限切れ) の     |
//|   金銭的結果がすべて同じため、最大化すべきは P(通過) のみ。        |
//|   実機+0.25%・残り41営業日からのMC (30,000パス):                   |
//|     mult 2.5 → 通過 23.7〜25.3% / 4.0 → 31.7〜44.3%               |
//|     (幅はBalGuard執行力の悲観〜楽観バウンド。5.0は悲観側で頭打ち)  |
//|   ⚠歴史最悪日×4.0 = -4.44%。悪い月曜の生存は BalGuard(-1.5%全決済) |
//|     の執行力に全面依存する。ガード類の校正値は変更しない。         |
//|   ⚠途中ロット増の規約適合 (一貫性/ギャンブル判定) は要確認。       |
//|                                                                   |
//|   (v1.03 — 2026-08-14 コードレビュー反映版(2系統統合))             |
...
#property strict
#property description "[RecentFit C6m Fintokei 6078225] Mon GBPJPY+AUDJPY / v4 NZDUSD+AUDUSD. mult 4.0 (challenge P-max, user decision 2026-08-18). Floor -2.4 (tick), daily -1.2 stop / -1.5 close (UTC0, max(bal,eq) anchor), day profit +2.5 stop / +2.9 close, lock 5.9/6.15. Expiry 2026-10-13 UTC."
...
input double InpMult     = 4.0;    // v1.04: P(通過)最大化(2.5校正値→引き上げ。§14-2)
...
int OnInit()
{
   ...
   PrintFormat("[INIT RecentFit C6m v1.04] initBal=%.0f mult=%.1f Σw=%.3f (グロス想定≈%.1fx) floor=-%.1f%% daily=-%.1f/-%.1f%% cap=+%.1f%% lock=%.2f/%.2f expiry=%s Magic=%I64d/%I64d/%I64d",
...
void OnTimer()
{
   ...
   Comment(StringFormat("Fintokei_RecentFit_C6m v1.04 | gain %+.2f%% | mult %.1f | %s",gainPct,InpMult,
