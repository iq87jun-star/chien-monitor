...
// 必要な環境変数: POKECA_X_API_KEY, POKECA_X_API_SECRET(ポケカbot専用アプリのキー。
//                未設定なら共通の X_API_KEY / X_API_SECRET を使う)、
//                POKECA_X_ACCESS_TOKEN, POKECA_X_ACCESS_TOKEN_SECRET(bot垢のトークン)
...
export async function postToX() {
  // ポケカbot専用アプリのキーを優先し、無ければ共通のアプリキーを使う。
  // Access Tokenは「apiKey側のアプリで発行されたもの」でないと認証が通らない点に注意。
  const apiKey = process.env.POKECA_X_API_KEY || process.env.X_API_KEY;
  const apiSecret = process.env.POKECA_X_API_SECRET || process.env.X_API_SECRET;
  const { POKECA_X_ACCESS_TOKEN, POKECA_X_ACCESS_TOKEN_SECRET } = process.env;
  if (!apiKey || !apiSecret || !POKECA_X_ACCESS_TOKEN || !POKECA_X_ACCESS_TOKEN_SECRET) {
  ...
  const creds = {
    apiKey,
    apiSecret,
    accessToken: POKECA_X_ACCESS_TOKEN,
    accessSecret: POKECA_X_ACCESS_TOKEN_SECRET,
  };
