...
const SYSTEM_PROMPT = `あなたはポケモンカード(ポケカ)の「国内市場価格」の日本語アナリストです。渡された集計データだけを根拠に、日本語の相場解説記事を書きます。
...
- 対象は未開封BOX(シュリンク付き)と高額シングルカード・プロモ。読者は日本のポケカプレイヤー・コレクター。
...
export async function generateArticles() {
  ...
  const userPrompt = `以下はポケモンカード国内市場(楽天市場の出品価格)の最新集計データです(lowJpyは最安出品価格、medianJpyは出品中央値、listingCountは集計対象の出品数、msrpJpyは定価、premiumPctは定価比の上乗せ率%、change7dPctは約7日前の中央値との比較%)。
このデータから、今のポケカ国内相場の解説記事を1本書いてください。セクションは2〜4個。gainers/losersが空の場合はプレミア率・高額アイテム・出品僅少を中心に書いてください。
