...
// 必要な環境変数: GUNPLA_X_API_KEY, GUNPLA_X_API_SECRET(ガンプラbot専用アプリのキー。
//                未設定なら共通の X_API_KEY / X_API_SECRET を使う)、
//                GUNPLA_X_ACCESS_TOKEN, GUNPLA_X_ACCESS_TOKEN_SECRET(bot垢のトークン)
import fs from "node:fs/promises";
import path from "node:path";
import crypto from "node:crypto";
import { SITE_DIR, CONTENT_DIR, SIGNIFICANT_CHANGE_PCT } from "./config.mjs";

const SITE_URL = "https://gunpla-souba.com/";
const MAX_POSTS_PER_RUN = 2;
const HASHTAGS = "#ガンプラ #ガンプラ再販";
...
export async function postToX() {
  // ガンプラbot専用アプリのキーを優先し、無ければ共通のアプリキーを使う。
  // Access Tokenは「apiKey側のアプリで発行されたもの」でないと認証が通らない点に注意。
  const apiKey = process.env.GUNPLA_X_API_KEY || process.env.X_API_KEY;
  const apiSecret = process.env.GUNPLA_X_API_SECRET || process.env.X_API_SECRET;
  const { GUNPLA_X_ACCESS_TOKEN, GUNPLA_X_ACCESS_TOKEN_SECRET } = process.env;
  if (!apiKey || !apiSecret || !GUNPLA_X_ACCESS_TOKEN || !GUNPLA_X_ACCESS_TOKEN_SECRET) {
  ...
  const creds = {
    apiKey,
    apiSecret,
    accessToken: GUNPLA_X_ACCESS_TOKEN,
    accessSecret: GUNPLA_X_ACCESS_TOKEN_SECRET,
  };
