...
## Portfolio3 比率×サイズ 正式最適化（docs/49）— 「40:40:20 は既にほぼ最適」を確定

3戦略(v7:v4:E5)比率は 40:40:20 が手置きのまま未スイープだった（docs/45 §3）。各戦略の Sharpe を確定10年値
（v7 0.78 / v4 0.74 / E5 0.57）にアンカーし分布形状は実系列から保持、リスク配分×サイズを
[`research/optimize_portfolio3.py`](research/optimize_portfolio3.py) で目標 p95 maxDD=−8% にスイープした。

| 比率 v7:v4:E5 | CAGR | 5年失格 | Calmar |
|---|--:|--:|--:|
| 100:0:0（v7単体） | 3.12% | 1.18% | 0.392 |
| **40:40:20（現状・手置き）** | **5.92%** | **1.25%** | **0.744** |
| 40:35:25（★Calmar最大） | 6.11% | 1.17% | **0.769** |
| 45:35:20（★CAGR最大） | 6.12% | 1.27% | 0.769 |

→ **手置き 40:40:20 は正式スイープでも実質フロンティア上**（最適点との差は CAGR約0.2%・失格約0.1%＝誤差内）。
3戦略は Sharpe近接＋ほぼ無相関ゆえ**比率に頑健＝大きな再配分は不要**。詰めるなら **40:35:25**、ただし
