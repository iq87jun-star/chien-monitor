...
2026-08-28 決定。3案から A(碁盤+石)を採用し、小さいサイズでも
格子が残るよう線を太くした版を `mark_go.svg` として作成した。

| ファイル | 用途 |
|---|---|
| `brand/mark_go.svg` | 原本 |
| `brand/go_1024.png` | 高解像度 |
| `brand/go_512.png` | 一般的なプロフィール画像 |
| `brand/go_256.png` | 一覧表示 |
| `brand/go_128.png` `go_96.png` | 小サイズ |

## なぜこの絵か

**定石(じょうせき)は囲碁の用語**で、隅で確立された打ち方の型を指す。
商品名の「定石 零 / 壱 / 弐」はここから取っている。

絵は碁盤の格子に星、そして**隅に置かれた白石**。
定石が隅の打ち方であることと一致している。

不採用にした2案:
- B(棒グラフ): 囲碁と無関係。曜日別収益の図に見える
- C(十字に石): 十字が照準に見え、トレード商品では別の意味を持ってしまう
