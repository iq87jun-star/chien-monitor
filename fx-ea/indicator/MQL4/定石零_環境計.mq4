...
int      g_samples     = 0;    // 中央値の算出に使えた総サンプル数
// --- 自前のスプレッド収集 ---
// 業者によっては履歴バーにスプレッドが入っていない(MetaQuotes-Demo等)。
// その場合でも使えるよう、毎分の実測値を自分で貯めてファイルに残す。
int      g_live[24][MAX_LIVE];
int      g_liveCnt[24];
int      g_livePos[24];
datetime g_lastSample  = 0;
datetime g_lastSave    = 0;
bool     g_liveLoaded  = false;
...
int OnCalculate(const int rates_total, const int prev_calculated,
                ...
                const long &volume[], const int &spread[])
  {
   ...
   // 毎分1件、実測スプレッドを自分で貯める
   if(now - g_lastSample >= 60)
     {
      SampleSpread(now);
      g_lastSample = now;
     }
   // 10分ごとにファイルへ保存(再起動しても蓄積が消えないように)
   if(now - g_lastSave >= 600)
     {
      SaveLive();
      g_lastSave = now;
     }
...
string LiveFileName()
  {
   return("ChienCM_" + _Symbol + ".csv");
  }

//+------------------------------------------------------------------+
//| 現在のスプレッドを1件、その時刻の時間帯に貯める                   |
//+------------------------------------------------------------------+
void SampleSpread(const datetime now)
  {
   ...
   g_live[h][g_livePos[h]] = (int)sp;
   g_livePos[h] = (g_livePos[h] + 1) % MAX_LIVE;      // 古いものから上書き
   if(g_liveCnt[h] < MAX_LIVE) g_liveCnt[h]++;

   g_medValid = false;   // 次の描画で組み直させる
  }

//+------------------------------------------------------------------+
void SaveLive()
  {
   int fh = FileOpen(LiveFileName(), FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
   if(fh == INVALID_HANDLE) return;
   for(int h = 0; h < 24; h++)
      for(int k = 0; k < g_liveCnt[h]; k++)
         FileWrite(fh, h, g_live[h][k]);
   FileClose(fh);
  }
...
void LoadLive()
  {
   if(g_liveLoaded) return;
   g_liveLoaded = true;
   if(!FileIsExist(LiveFileName())) return;
   int fh = FileOpen(LiveFileName(), FILE_READ | FILE_CSV | FILE_ANSI, ',');
   if(fh == INVALID_HANDLE) return;
   while(!FileIsEnding(fh))
     {
      int h  = (int)FileReadNumber(fh);
      if(FileIsEnding(fh)) break;
      int sp = (int)FileReadNumber(fh);
      if(h < 0 || h > 23 || sp <= 0) continue;
      if(g_liveCnt[h] >= MAX_LIVE) continue;
      g_live[h][g_liveCnt[h]] = sp;
      g_liveCnt[h]++;
      g_livePos[h] = g_liveCnt[h] % MAX_LIVE;
     }
   FileClose(fh);
  }
...
void BuildSpreadProfile()
  {
   ...
   // 自前で貯めた実測値を合流させる(業者の履歴にスプレッドが無い場合の主役)
   for(int h = 0; h < 24; h++)
      for(int k = 0; k < g_liveCnt[h] && cnt[h] < MAX_PER_HOUR; k++)
        {
         g_buf[h][cnt[h]] = g_live[h][k];
         cnt[h]++;
        }
