...
// 必要な環境変数: MTG_X_API_KEY, MTG_X_API_SECRET(mtg専用アプリのキー。
//                未設定ならゲーム系ボット共通の X_API_KEY / X_API_SECRET を使う)、
//                MTG_X_ACCESS_TOKEN, MTG_X_ACCESS_TOKEN_SECRET(bot垢のトークン)
import fs from "node:fs/promises";
import path from "node:path";
import crypto from "node:crypto";
import { SITE_DIR, CONTENT_DIR, SIGNIFICANT_CHANGE_PCT } from "./config.mjs";

const SITE_URL = "https://mtg-kaigai.com/";
const MAX_POSTS_PER_RUN = 2;
const HASHTAGS = "#MTG #マジックザギャザリング";
...
export async function postToX() {
  // mtg専用アプリのキーを優先し、無ければゲーム系ボット共通のアプリキーを使う。
  // Access Tokenは「apiKey側のアプリで発行されたもの」でないと認証が通らない点に注意。
  const apiKey = process.env.MTG_X_API_KEY || process.env.X_API_KEY;
  const apiSecret = process.env.MTG_X_API_SECRET || process.env.X_API_SECRET;
  const { MTG_X_ACCESS_TOKEN, MTG_X_ACCESS_TOKEN_SECRET } = process.env;
  if (!apiKey || !apiSecret || !MTG_X_ACCESS_TOKEN || !MTG_X_ACCESS_TOKEN_SECRET) {
  ...
  const creds = {
    apiKey,
    apiSecret,
    accessToken: MTG_X_ACCESS_TOKEN,
    accessSecret: MTG_X_ACCESS_TOKEN_SECRET,
  };
  ...
  if (spike) {
    ...
    queue.push({
      kind: "spike",
      key: spike.id,
      text: `📈【MTG海外相場 急騰】\n${spike.name}(${spike.set})\n$${spike.usd}(${jpy})・7日前比 +${spike.change7d}%\n\n海外市場(TCGplayer)の値動きを毎日自動集計→ ${SITE_URL}\n${HASHTAGS}`,
    });
  }
