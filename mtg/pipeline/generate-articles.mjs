...
const SYSTEM_PROMPT = `あなたはマジック:ザ・ギャザリング(MTG)の「海外市場」の日本語アナリストです。渡された集計データだけを根拠に、日本語の解説記事を書きます。

対象データの前提(記事内でも正しく扱うこと):
- 価格は英語版カードの「海外市場価格」。TCGplayer(米国最大のトレカマーケット)のドル建て価格と、その円換算値(参考値)。日本国内のショップ買取・販売価格ではない。
- 監視対象は直近発売の数セットのみ(全カードプールではない)。
- change7dPct は自前の価格履歴による約7日前との乖離(%)。
- foilOnly が true のカードは通常版の価格が無くFoil版の価格を採用している。
- 読者は日本のMTGプレイヤー・コレクター・海外へ販売するセラー。「海外でいくらで取引されているか」を知りたい層。
...
- 通貨単位を必ず明記する(ドルは$、円換算は「約◯円」のように参考値と分かる書き方)。
- カード名・セット名はデータ内の英語表記(API由来)をそのまま使う。
- 憶測で断定しない。データから直接読み取れない因果関係(大会環境・禁止改訂・再録等)は「〜が要因とみられる」など推測と分かる表現にする。
...
export async function generateArticles() {
  ...
  const input = {
    ...
    sets: economy.sets.map((s) => ({
      name: s.name,
      releaseDate: s.releaseDate,
      topCard: s.topCard && {
        name: s.topCard.name,
        priceUsd: s.topCard.usd,
        priceJpyApprox: s.topCard.jpy,
      },
    })),
    fxUsdJpy: economy.fx?.usdJpy ?? null,
  ...
  const userPrompt = `以下はMTG英語版カードの海外市場(TCGplayer)の最新集計データです(priceUsdはドル建て価格、priceJpyApproxはECB為替レートによる円換算の参考値、change7dPctは約7日前の価格との乖離率%)。監視対象は直近発売のセット群です。
このデータから、今の海外市場動向の解説記事を1本書いてください。セクションは2〜4個。
