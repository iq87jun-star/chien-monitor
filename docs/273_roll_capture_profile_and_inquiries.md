...
| 非 FX Hold+v4+Mon ×1.48 | docs/182 | — | — | — | 常時 LONG(ベータ)+Mon | FN #14166201(swap-free)で稼働 → 2026-09-25 v1.30 で **ロール ×3 を内蔵**(docs/241 §1e) |
...
| FundedNext | 9/22 01:39 UTC(support@) | 9/22 03:19 UTC 回答 + **9/24 04:38 UTC 再確認回答**(docs/185 §10b-3): ①の "extra charge" は購入時のアドオン料金(+10%)の意味で、**保有中の追加費用は無い**と明記。③ abuse でない・④資金化後も継続 | **条件クリア**。#14166201(アドオン購入済み)はロールレグの受け皿候補。FN は 1 口座 1 EA のため NonFX EA への内蔵が必要(docs/275 の NonFX+Roll 構成)。採否は 10/1 監視後に Fintokei スイングと合わせて判断 |
