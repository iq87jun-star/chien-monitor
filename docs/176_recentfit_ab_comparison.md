...
- 規律: **tradingethicsの回答が届くまで購入は保留**(チャット2回の「非該当」は心証であり
  確定ではない)。
- ⚠【追記 2026-07-31】チャットはその後タイムアウトで自動終了。終了通知の送信者表示が
  「**FundedNext AI・AI Agent**」であり、Richard氏の一連の回答は**AIエージェントの可能性が高い**。
  防衛材料としての重みはさらに割引く(=tradingethicsメール回答の必要性が一段上がった)。
  人間の担当への到達手段はメール(tradingethics)一択とする。
...
### 7.8【両案同時稼働プラン】(2026-07-31 ユーザー意向: A/B両方を稼働させたい)

規約制約(§7.5-7.7)から、両稼働の配置は実質1通りに定まる:

| 案 | 配置 | 根拠 |
|---|---|---|
| **A案(RecentFit5)** | **FN Stellar $50k** | tradingethics回答待ちのみ。Max Risk3%は取引個別=1.25%フル速度可。EV最大 |
| **B案(RecentFit 2026H2)** | **Fintokeiチャレンジ(新口座)** | FN/FTMOはv7同族Monレッグの重複が濃く不可。Fintokeiは同社内EA並走の書面OK+銘柄重複は同方向のみ(両建て非該当)。トレードグループ3%が制約 |

#### B案のFintokei適合に必要な改修(稼働前必須)

1. **倍率再校正**: 4.8xの生の最悪日−2.79%はグループ警告線3%に接触圏 → 倍率を≈4.0xへ
   縮小するか、グループ実損−2.5%全決済ガードを追加(§7.5.2の確定仕様で実装可能)。
2. **MC再実行**: P1+8%/P2+6%(FN5%でなく)・equity日次−5%・警告回避ガード込みで
   通過率/所要日数を再計算(4.8x→4.0x減速も織り込む)。
3. EAプリセット: 初期残高JPY・ロック値(Fintokei目標に合わせ8.05/P2は6%系へ)・
   JP225スワップ/配当の業者実測(元々の残タスク)。
4. docs番号の衝突解消: A/B両採用なら片方のdocs/174-175を177-178へ振り直してからマージ。

#### 7.8.1 変形案: 両方FN(2026-07-31 ユーザー打診)

**チャレンジ同士は障害にならない**: 本人名義のFNチャレンジ口座間は同一EA・コピートレード
すら公式可(docs/161 §5)。A案とB案が相互にGBPJPY月曜で重なっても、チャレンジ2口座なら
規約上の問題はない。**唯一の障害はInstant口座のv7**(Instant↔チャレンジ間の同一取引禁止):

- A案: S2のみが接触(GBPJPY 08:00) — tradingethics回答待ちの現行シナリオ
- B案: **Monレッグ(GBPJPY+AUDJPY×4/6/8/10時)がv7と同族・同銘柄・同時刻** — 接触が最も濃い

成立ルートは2つ:

| ルート | 内容 | 評価 |
|---|---|---|
| ① tradingethicsの広い「可」 | 回答が「銘柄・方向・時刻が重なっても、別戦略・別ロット・別リスクなら可」と読める場合 → B案シナリオ(GBPJPY+AUDJPY・4時刻一致)を**明示した追加確認**を同スレッドで取ってから両方購入 | 回答文言次第。B案は「別戦略」の主張が弱い(Monレッグはv7と同一構築式)ため、Aが通ってもBは個別確認必須 |
| ② Instant側のv7を無効化 | Instant PD EAのv7レッグをOFF(またはInstant停止) → Instant↔チャレンジの重複が**物理的に消滅**し、書面回答に依存せず両方FNが成立 | 構造的に最も確実。**ただし正攻法トラックの確定配置(docs/172)を宝くじトラックの都合で変える=トラック分離原則への抵触**。Instant(20k・昇格中央19ヶ月)の主力レッグ(35.7%)を失う代償 |

- 数字の比較: Instant v7の逸失(20k口座の年率+5.1%の主要部分) vs チャレンジ2枚のEV
  (≈+$1,770×2、§7.4)。**額ではチャレンジ2枚が圧倒的に大きい**が、②は正攻法への介入で
  あり数字だけで決めない(docs/174の「トラック間の資金・判断の混線禁止」に照らして
  ユーザー判断事項)。
- B案をFNに載せる場合の実務利点: **改修ゼロ**(EA/MC/プリセットが元々FN Stellar校正。
  §7.8のFintokei適合改修1-3が全て不要)。docs番号の振り直し(§7.8 #4)のみ残る。
- 推奨手順: tradingethics回答を待つ → 広い「可」ならB案シナリオを明示で追い確認 →
  それもクリアなら両方FN(§7.8のFintokei行きは取り消し)。どちらかで詰まったら
  B=Fintokei(§7.8)へ戻る。②のInstant v7無効化は「どうしても両方FN」の場合の最終手段。

#### 7.8.2 参考試算: Instant適応時の期待収益(2026-07-31 ユーザー要望)

**前提の制約(重要)**: FN Stellar Instantは**購入合算上限$20k**(docs/154・2026-07-15公式Help
確認済み)。既存Instant 20kで上限到達済みのため**2枚目のInstantは買えない**。
「Instant適応」=既存20kのPD_noV4を置き換えること(=正攻法配置docs/172の変更。§7.8.1②と
同じくトラック分離原則への抵触をユーザーが受容する場合のみ)。

試算前提: $20k・分配70%(tier1-2。tier3+で80%)・USDJPY159.3・掛金$599は支払済み(返金なし)。
年率は各案の研究値(楽観=直近窓ブートストラップ。**実力と読まない**こと)。

| シナリオ | A案(リスク1.25%) | B案(4.8x) |
|---|--:|--:|
| 楽観(直近窓年率) | +25.0%→粗利$5,000→**手取≈¥55.7万/年**(月¥4.6万) | +51.6%→$10,320→**手取≈¥115万/年**(月¥9.6万) |
| 減衰(エッジ半減/全期間参考) | +12.5%→**≈¥27.9万/年** | 年率≈+25-30%→**≈¥56-67万/年** |
| 消滅/最悪 | ≈0(MLL−6%内で停止・$599喪失) | **2018型年(−7.7%)はトレーリング−6%接触=口座喪失** |

適合性の注記:
- A案はmaxDD−1.1%(楽観窓)でトレーリング−6%に余裕。EAガードのフロアを−8%→−5%(HWM基準)へ
  変更する改修が必要(v1.43系トレーリングガードの流用で可)。
- B案は月中DD生−4.62% vs MLL−6%=**余白1.4%のみ**。4.8xが上限・安全側は4.0x(数字×5/6)。
  全期間MCの失格25%(静的−10%前提)より**トレーリング−6%での喪失確率は高い**。
- ⚠ B案のJP225 HoldはFN100kチャレンジの季節(指数)と接触し得る(Instant↔チャレンジ禁止)
  → Holdレッグ(重み7%)を外して再正規化するのが安全。
- A案をInstantに載せた場合、FN100k季節(指数/金)との接触はゼロ。ただし**チャレンジ側に
  A/B案を並行配備するとInstant↔チャレンジのGBPJPY月曜接触が再発生**する(配置は排他的に
  考えること)。

対照(チャレンジ経由・$50k資金化後・分配80%): A案≈¥159万/年 / B案≈¥329万/年(楽観)。
**Instant適応の手取りはチャレンジ経由の1/3以下**(口座20k+分配70%のため)。Instantの利点
「審査なし即稼働」は、$20k上限と正攻法配置の破壊を代償にしてまで取る規模ではない、が数字上の結論。

#### 7.8.3【配置案 2026-07-31】ユーザー提案: FN現状維持+FTMO=新①+Fintokei=新②

提案: FN Instant=PD(現状) / FN100k=季節(現状) / FTMO=新①(A案想定) / Fintokei=新②(B案想定)。

**規約評価: この配置の最大の利点は新規2枚がFN外にあること** — Instant↔チャレンジの
同一取引問題(§7.6系)が構造的に消滅し、**tradingethics回答が購入のブロッカーでなくなる**。

| 接触点 | 判定 |
|---|---|
| FN内(Instant PD↔FN100k季節) | 現状のまま=docs/172で適合確認済み |
| FTMO新①↔FN Instant PD(v7) | **別業者・独立EA=可**(FN禁止はコピーツール連結のみ, docs/161 §5) |
| Fintokei新②↔FN100k季節(JP225等) | **別業者=可**(Fintokei書面: 他社並走OK, docs/173) |
| FTMO新①↔Fintokei新②(GBPJPY月曜) | 別業者=可 |

**残る確認・決定事項(3点)**:
1. **置き換えか追加か**:
   - FTMO: 既存PD 2.0x口座を新①に**置き換え**ならFTMO内はクリーン。PDを残して**新規口座追加**
     なら新①S2とPD v7のGBPJPY月曜重複がFTMO内に発生 → FTMOサポートへ本人口座間の扱いを
     書面確認(当方に書面なし)。
   - Fintokei: パール季節RG3を**置き換え**でも、パールを残して**新規口座追加**でも規約はクリア
     (複数口座EA並走の書面OK・全ロングで両建て非該当, §7.5)。
2. **FTMO口座タイプ**: 新①(A案)はS4/S5が3〜5日保有=週末持越あり → **Swing必須**
   (Standardは週末持越不可, docs/94 §3)。既存口座の種別を確認。
   規則の型はFN Stellarと同型(P1+8/P2+5/静的−10/日次−5)なのでA案EAはほぼ無改修で移植可。
3. **新②(B案)のFintokei適合**(§7.5系の帰結):
   - **JP225 Holdレッグは外す**(常時保有だとトレードグループが永久にリセットされず、
     確定損の累積で3%警告線に必ず到達する構造)。Mon/v4のみなら24h/≤8日で自然にフラット化
     しグループがリセットされる。構成はInstant版と同じ再正規化3レッグを流用。
   - 倍率4.8→**4.0x**(グループ3%警告の回避余白)・P2目標6%に合わせLock値変更・MC再計算。
   - Prop版EAはレッグCSV入力なので**プリセット変更のみで対応可**(コード改修不要)。

**正直な指摘**: 置き換え読みの場合、正攻法からFTMO PD 2.0x(到達95.2%/年率10%)と
購入直後のパール季節RG3パイロット(docs/173)が消える=docs/172体制の半分を宝くじトラックに
明け渡す配置になる。追加読みなら費用はFTMO100k≈$540+Fintokei¥1.25万〜(どちらも別途)。
7.8.2のInstant置換案は本配置では**不要になる**(Instant=PD維持)。

- **分散にはならない**: 両案ともGBPJPY月曜LONGを含み、直近レジーム継続という同一仮定に
  賭けている。相関した2枚 = 当たれば2口座資金化、外れれば2枚同時に沈む「2倍掛け」。
- **費用**: FN50k $299.99(≈¥4.8万)+Fintokei(サイズにより¥1.25万〜) ≈ 合計¥6〜9万。
  「チャレンジ2回分で打ち止め」予算を両案合算で適用するなら、**この購入で予算を使い切る**
  (どちらか失格しても買い直しなし・両方失格でトラック終了)。
- **順序**: FN tradingethics回答→A案購入。B案はFintokei適合改修+MC再実行が済んでから
  (同時に買う必要はない。A案を先行させ、B案は8月中旬目処でも期限2026-10-31に間に合う)。
