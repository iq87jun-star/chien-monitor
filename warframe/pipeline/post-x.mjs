...
export async function postToX() {
  const {
    X_API_KEY,
    X_API_SECRET,
    WARFRAME_X_ACCESS_TOKEN,
    WARFRAME_X_ACCESS_TOKEN_SECRET,
  } = process.env;
  if (!X_API_KEY || !X_API_SECRET || !WARFRAME_X_ACCESS_TOKEN || !WARFRAME_X_ACCESS_TOKEN_SECRET) {
  ...
  const creds = {
    ...
    accessToken: WARFRAME_X_ACCESS_TOKEN,
    accessSecret: WARFRAME_X_ACCESS_TOKEN_SECRET,
  };
  ...
  if (state.lastRankingDate !== todayJst && top3.length === 3) {
    ...
    queue.push({
      kind: "ranking",
      key: todayJst,
      text: `📈 プライム相場 本日の急騰TOP3\n\n${lines}\n\n※7日変動率・毎日自動集計(warframe.market)\n詳細→ ${SITE_URL}\n${HASHTAGS}`,
    });
  }
