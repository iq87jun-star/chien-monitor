...
// ダークミリタリー(オリーブ/カーキ)配色。タルコフの雰囲気に合わせる
const T = {
  bg: "#0D0F0A",
  surface: "#171A12",
  surfaceVar: "#20241A",
...
const fmtRub = (v) => `${Math.round(v).toLocaleString("ja-JP")} ₽`;
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11, color: T.textLow }}>{item.shortName}</div>
      </div>
      <div style={{ textAlign: "right" }}>
        <div style={{ fontSize: 13, color: T.accentLight, fontWeight: 600 }}>
          {fmtRub(item.price)}
        </div>
        <Change pct={item.change48h} />
      </div>
    </div>
  );
}
    ...
    >
      ...
      >
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 12, color: T.textMed, marginTop: 4 }}>
            Escape from Tarkov フリーマーケット相場 ・ 最終更新 {fmtDate(economy.generatedAt)} ・
            価格は24時間平均(₽)、変動率は直近48時間
          </div>
        </div>
      </header>
      ...
      >
        ...
        {available ? (
          ...
          >
            <Card title="📈 高騰アイテム TOP10" sub="直近48時間の上昇率(平均価格1万₽以上)">
              {economy.gainers.map((i) => (
                <ItemRow key={i.name} item={i} />
              ))}
            </Card>
            <Card title="📉 下落アイテム TOP10" sub="直近48時間の下落率(平均価格1万₽以上)">
              {economy.losers.map((i) => (
                <ItemRow key={i.name} item={i} />
              ))}
            </Card>
            <Card title="👑 高額アイテム TOP10" sub="現在の24時間平均価格">
          ...
          <Card title="🔧 メンテナンス情報">
            <p style={{ margin: 0, fontSize: 13, color: T.textMed, lineHeight: 1.8 }}>
              データ取得元(tarkov.dev)が一時的に利用できません。次回の自動更新で復旧します
            </p>
          </Card>
        )}
        ...
        >
          データ出典:{" "}
          <a href="https://tarkov.dev" style={{ color: T.textMed }}>
            tarkov.dev
          </a>
          (公開GraphQL API)。
          本サイトはBattlestate Gamesとは無関係の非公式ファンサイトです。
