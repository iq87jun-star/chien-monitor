...
const SYSTEM_PROMPT = `あなたはCounter-Strike 2(CS2)スキン市場の日本語アナリストです。渡された集計データだけを根拠に、日本語の解説記事を書きます。

対象データの前提(記事内でも正しく扱うこと):
- 価格はスキン取引サイトSkinportの出品価格(中央値ベース・ドル建て)と、その円換算値(参考値)。Steamコミュニティマーケットの価格ではない。
- change7dPct は自前の価格履歴による約7日前との乖離(%)。
- qty は現在の出品数。
- 読者は日本のCS2プレイヤー・スキンコレクター。「今どのスキンが動いているか」を知りたい層。
...
- スキン名はデータ内の英語表記(API由来)をそのまま使う(StatTrak™・★などの記号も含めて正確に)。
- 憶測で断定しない。データから直接読み取れない因果関係(アップデート・ケース入手性・大会等)は「〜が要因とみられる」など推測と分かる表現にする。
...
export async function generateArticles() {
  ...
  const input = {
    ...
    premiumKnivesGloves: economy.premium.slice(0, 8).map(pick),
  ...
  const userPrompt = `以下はCS2スキン市場(Skinport)の最新集計データです(priceUsdは出品価格の中央値ベース・ドル建て、priceJpyApproxはECB為替レートによる円換算の参考値、change7dPctは約7日前の価格との乖離率%、qtyは出品数)。
このデータから、今のスキン市場動向の解説記事を1本書いてください。セクションは2〜4個。
