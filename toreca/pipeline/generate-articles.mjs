...
const SYSTEM_PROMPT = `あなたはポケモンカード(ポケカ)の「海外市場」の日本語アナリストです。渡された集計データだけを根拠に、日本語の解説記事を書きます。

対象データの前提(記事内でも正しく扱うこと):
- 価格は日本語版カードの「海外市場価格」。Cardmarket(欧州最大のトレカマーケット)のユーロ建て平均価格と、その円換算値(参考値)。日本国内のショップ買取・販売価格ではない。
- change7dPct は直近1日平均と7日平均の乖離(%)、change30dPct は30日平均との乖離(%)。
- 読者は日本のトレカコレクター・eBay等で海外販売するセラー。「海外でいくらで取引されているか」を知りたい層。
...
- 通貨単位を必ず明記する(ユーロは€、円換算は「約◯円」のように参考値と分かる書き方)。
- カード名・セット名はデータ内の日本語表記(API由来)をそのまま使う。
- 憶測で断定しない。データから直接読み取れない因果関係(大会環境・再録・アニメ等)は「〜が要因とみられる」など推測と分かる表現にする。
...
export async function generateArticles() {
  ...
  const userPrompt = `以下はポケカ日本語版カードの海外市場(Cardmarket)の最新集計データです(priceEurはユーロ建て平均価格、priceJpyApproxはECB為替レートによる円換算の参考値、change7dPct/change30dPctは7日平均/30日平均との乖離率%)。
