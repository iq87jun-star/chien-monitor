...
// 必要な環境変数: DUEL_X_API_KEY, DUEL_X_API_SECRET(遊戯王bot専用アプリのキー。
//                未設定ならゲーム系ボット共通の X_API_KEY / X_API_SECRET を使う)、
//                DUEL_X_ACCESS_TOKEN, DUEL_X_ACCESS_TOKEN_SECRET(bot垢のトークン)
import fs from "node:fs/promises";
import path from "node:path";
import crypto from "node:crypto";
import { SITE_DIR, CONTENT_DIR, SIGNIFICANT_CHANGE_PCT } from "./config.mjs";

const SITE_URL = "https://pocketduel.tokyo/";
...
export async function postToX() {
  // 遊戯王bot専用アプリのキーを優先し、無ければゲーム系ボット共通のアプリキーを使う。
  // Access Tokenは「apiKey側のアプリで発行されたもの」でないと認証が通らない点に注意。
  const apiKey = process.env.DUEL_X_API_KEY || process.env.X_API_KEY;
  const apiSecret = process.env.DUEL_X_API_SECRET || process.env.X_API_SECRET;
  const { DUEL_X_ACCESS_TOKEN, DUEL_X_ACCESS_TOKEN_SECRET } = process.env;
  if (!apiKey || !apiSecret || !DUEL_X_ACCESS_TOKEN || !DUEL_X_ACCESS_TOKEN_SECRET) {
  ...
  const creds = {
    apiKey,
    apiSecret,
    accessToken: DUEL_X_ACCESS_TOKEN,
    accessSecret: DUEL_X_ACCESS_TOKEN_SECRET,
  };
  ...
  if (spike) {
    ...
    queue.push({
      kind: "spike",
      key: spike.id,
      text: `📈【遊戯王海外相場 急騰】\n${spike.name}\n€${spike.eur}(${jpy})・7日前比 +${spike.change7d}%\n\n海外市場(Cardmarket/TCGplayer)の値動きを毎日自動集計→ ${SITE_URL}${UTM("spike")}\n${HASHTAGS}`,
    });
  }
