...
- カード名は英語(TCG)表記 — eBay等の海外取引で実際に使う名称のため、輸出セラーには
  むしろ実用的
- ブラウザ拡張「トレカ海外相場チェッカー」(`extension/`)用に、ビルド時に
  `dist/api/cards.json`(€0.30以上のカード・日本語名つき)を出力する。日本語名は
  [db.ygoresources.com](https://db.ygoresources.com) の名前索引から作る英語名→日本語名の
  対応表(`data/raw/ja-names.json`・変化があった時だけ更新)
