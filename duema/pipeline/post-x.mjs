...
// 必要な環境変数: DUEMA_X_API_KEY, DUEMA_X_API_SECRET(デュエマbot専用アプリのキー。
//                未設定なら共通の X_API_KEY / X_API_SECRET を使う)、
//                DUEMA_X_ACCESS_TOKEN, DUEMA_X_ACCESS_TOKEN_SECRET(bot垢のトークン)
import fs from "node:fs/promises";
import path from "node:path";
import crypto from "node:crypto";
import { SITE_DIR, CONTENT_DIR, SIGNIFICANT_CHANGE_PCT } from "./config.mjs";

const SITE_URL = "https://duema-souba.com/";
const MAX_POSTS_PER_RUN = 1;

// JST の暦日を YYYY-MM-DD で返す(1日1回ガード用)
const jstToday = () =>
  new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().slice(0, 10);
const HASHTAGS = "#デュエマ #デュエルマスターズ";
...
export async function postToX() {
  // デュエマbot専用アプリのキーを優先し、無ければ共通のアプリキーを使う。
  // Access Tokenは「apiKey側のアプリで発行されたもの」でないと認証が通らない点に注意。
  const apiKey = process.env.DUEMA_X_API_KEY || process.env.X_API_KEY;
  const apiSecret = process.env.DUEMA_X_API_SECRET || process.env.X_API_SECRET;
  const { DUEMA_X_ACCESS_TOKEN, DUEMA_X_ACCESS_TOKEN_SECRET } = process.env;
  if (!apiKey || !apiSecret || !DUEMA_X_ACCESS_TOKEN || !DUEMA_X_ACCESS_TOKEN_SECRET) {
  ...
  const creds = {
    apiKey,
    apiSecret,
    accessToken: DUEMA_X_ACCESS_TOKEN,
    accessSecret: DUEMA_X_ACCESS_TOKEN_SECRET,
  };
